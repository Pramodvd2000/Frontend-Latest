...
const ServiceStatusModal = ({ data1, currentStatus, onCancel, onUpdateSuccess }) => {
  ...
  const onSubmit = async (formData) => {
    try {
      // Prepare request body based on the passed form data
      const requestBody = {
        id: data1, // Assuming the ID is coming from data1
        status: formData.status?.value,
        comments: formData.comment || "",
      };

      console.log("requestBody",requestBody)
      // Add attended_by (steward) if stewardId is provided
      if (formData.stewardID?.value) {
        requestBody.attended_by = formData.stewardID.value;
      }

      // // Add completed_by if status is completed and stewardId is not selected
      // if (formData.status?.value === "Completed" && !formData.stewardID?.value) {
      //   requestBody.completed_by = data1?.data1?.user_id; // Assuming `user_id` is in data1
      // }
