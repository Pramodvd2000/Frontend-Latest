...
let industriesOptions = [
  fetchx(API_URL + "/getIndustriesByHotel")
    .then((result) => result.json())
    .then((resp) => {
      industriesOptions = resp.data.map((item) => ({
        value: item.id,
        label: item.industry,
      }));
    }),
...
const Floor = ({ stepper2, data1 }) => {
  ...
  const defaultIndustry = {
    value: data1.industryID,
    label: data1.industry,
  };
  ...
  const onSubmit = (data) => {
    ...
    if (data.accountType !== null && data.accountName !== null) {
      ...
      let createmarketGroup = JSON.stringify({
        ...
        "industryID": data.industry ? data.industry.value : data.industryID,
  ...
  const handleReset = () => {
    reset({
      ...
      const handleChange1 = (selectedOption1) => {
        setIsActive(selectedOption1.value);
        setTimeout(() => {
            setreload(true);
        }, 1);
    };
