...
// ** Reactstrap Imports
import { Input, Card, Form, Label, Button, CardBody, CardTitle, CardHeader, InputGroup, InputGroupText, Row, Col } from "reactstrap";
...
import EditExtraGroup from './editExtraGroup';
...
const RoomType = () => {
  ...
  const [extraName, setExtraName] = useState([])
    ...
    const [extraValue, setExtraOption] = useState();
    ...
    const handleError = (message) => {
      ...
      const handleSuccess = () => {
        return MySwal.fire({
            title: 'Extra Group Added Successfully!',
            text: 'Extra Group Added Successfully !!',
            icon: 'success',
            customClass: {
    ...
    const [columnDefs, setColumnDefs] = useState([
        { headerName: 'groupExtra', field: 'extraGroupName', cellStyle: { 'text-align': 'center', 'background-color': '#F1E39B' }, headerClass: "text-center", maxWidth: 130 },
    ...
    const onSubmit = (data) => {
        ...
        if (
            // data.hotelID !== null &&
            data.groupExtra !== null
        ) {
            // console.log(data);
            let createasset = JSON.stringify({
                "extraIDs": extraValue,
                "groupExtra": data.groupExtra,


            });
            // console.log(createasset);
            let res = fetchx(API_URL + "/addGroupExtra", {
                method: "POST",
            ...
            <div>
                <Card>
                    <Accordion open={open} toggle={toggle}>
                        <AccordionItem>
                            <AccordionHeader targetId='1'><h4><b> Add Extra Group</b></h4></AccordionHeader>
                                    ...
                                    <CardBody>
                                        <Form onSubmit={handleSubmit(onSubmit)}>
                                            <Row>
                                                <Col md='4' sm='12'>
                                                    <div >
                                                        <Label className="form-label" for="groupExtra">
                                                        ...
                                                        <InputGroup className="input-group-merge">
                                                            <Controller
                                                                ...
                                                                name='groupExtra'
                                                                control={control}
                                                                placeholder='groupExtra'
                                                                render={({ field }) => <Input placeholder="Type Extra Group"
                                                                    //  pattern="[a-zA-Z]*" title="Type Only Alphabets" 
                                                                    required
                                                                    className={classnames({
                                                                        "is-invalid": data !== null && (data.groupExtra === null || !data.groupExtra.length)
                                                                    })} {...field} />}
