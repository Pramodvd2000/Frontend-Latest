...
const ValidationThirdPartyComponents = ({getordersAmount}) => {
    ...
    const [GuestNameNC, setNCGuestName] = useState('');
    ...
    function handlevoidKOT(row, index) {
        fetchx(API_URL + '/getPOSManagerslist?hotelID=1&storeID=' + localStorage.getItem('storeID'))
    ...
    const handlePlusClick2 = (index) => {
        const currentQty = parseInt(kotData.qty);
        const newQty = currentQty + 1;
        setKotData({ ...kotData, qty: newQty });
            ...
            .then((result) => {
                ...
                result['data'].map((row, index) => {
                    if (row.orderType === 'NC KOT') {
                        fetchx(API_URL + '/getPOSOrderDetailsByOrderID?hotelID=1&storeID=' + localStorage.getItem('storeID') + '&orderID=' + localStorage.getItem('orderID'))
                        .then(result => result.json())
                        .then(resp => {
                            // console.log(resp['data'][0]['guestName'])
                          setNCGuestName(resp['data'][0]['guestName'])
                          sessionStorage.getItem(resp['data'][0]['guestName'])
                        })
                        ...
                        // navigate('/apps/posconfiguration/DisplayBill')
                        // setTimeout(() => { navigate('/apps/posconfiguration/DisplayBill'); }, 10);
                        setTimeout(() => { navigate('/apps/posconfiguration/DisplayBillPayment'); }, 10);
                        // console.log("hello");
                    }
                });
    ...
    const handleonCancelmodify = () => {
        setmodifyqtymodal(false)
        reset();
    }

    const handleNCGuestNameChange = (e) => {
        setNCGuestName(e.target.value);
    };
                ...
                <Col xl='7' md='6' xs='12' >
                    ...
                    >
                        <ModalBody className='px-5 pb-2'>
                            ...
                            <Button className="me-1"  color="danger" onClick={() => setShowModal(false)}>
                                    No
                                </Button>
                                <Button  color="primary" className='bg-transparent' onClick={confirmSubmit}>
                                    Yes
                                </Button>
                            </div>
                        </ModalBody>
                                ...
                                <Table >
                                    <thead>
                                        <tr style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                                            ...
                                            <th style={{ paddingLeft: '12%' }} colSpan="2">
                                            {pastKOT1.length > 0 && (
                                            <Button color="danger" onClick={handleVoidBill} style={{ marginLeft: '10px' }}>
                                                Void Bill
                                            </Button>
                                            )}
                                                {/* <Button color="danger" onClick={handleVoidBill} style={{ marginLeft: '10px' }}>Void Bill</Button> */}
                                            </th>
        ...
        <div align='end' className='buttons'>
            ...
            >
                ...
                <ModalBody>
                    ...
                    <Col sm='12' className='mb-1'>
                        <div className="mb-1">
                            <Label className="form-label" htmlFor="managerID" style={{ fontSize: '15px', paddingRight: '12px' }}>
                                Select Manager
                            </Label>
                            <InputGroup className="input-group-merge">
                                <Controller
                                    ...
                                    render={({ field }) => (
                                        <Input
                                            ...
                                            {...field}
                                        >
                                            <option value="" hidden></option> {/* Empty option with no text */}
                                            {managerList.map((steward, index) => (
                                                <option key={index} value={steward.id}>
                                                    {steward.stewardName}
                                                </option>
                                            ))}
                                        </Input>
                    ...
                    <Col sm='12' className='mb-1'>
                    <Label className="form-label" htmlFor="passCode" style={{ fontSize: '15px', paddingRight: '12px' }}>
                                Enter Passcode
                            </Label>
                    <div className='text-center mb-2'>
                ...
                />
                {errors.password && <span className='text-danger'>Passcode is required</span>}
            </div>
                    </Col>
                    <Col sm='12' className='mb-1' >
                        <div className="mb-1">
    ...
    <ModalBody>
        <Form onSubmit={handleSubmit(onSubmitModifyItem)}>
            <Col style={{ marginLeft: '20px' }}>
                {kotData.length !== 0 && (
                    <tr key={kotData} style={{ width: '500px' }}>
                        <td style={{ margin: '6px 0', paddingLeft: '10px', width: '300px' }}>{kotData.itemName}</td>
                        <td style={{ margin: '6px 0', paddingLeft: '10px' }} colSpan={2}>
                            <InputNumber 
                                disabled={true} 
                                id={"modifyqty" + kotData} 
                                value={kotData.qty} 
                                defaultValue={kotData.qty}
                                upHandler={<Plus onClick={() => handlePlusClick2(kotData)} />}
                                downHandler={<Minus onClick={() => handleMinusClick2(kotData)} />}
                            />
                        </td>                              
                    </tr>
                )}
            ...
            {/* Conditionally render Manager and Passcode fields if kotData.qty is 0 */}
            {showManagerFields && (
                <>
                    <Col style={{ marginLeft: '30px' }} sm='12' className='mb-2'>
                            ...
                            render={({ field, fieldState: { error } }) => (
                                <div>
                                    <Select
                                        required
                                        isClearable
                                        options={managerList.map(steward => ({ value: steward.id, label: steward.stewardName }))}
                                        classNamePrefix="select"
                                        theme={selectThemeColors}
                                        style={{ width: '400px' }}
                                        {...field}
                            ...
                            <Controller
                                ...
                                render={({ field }) => (
                                    <Input
                                        type='password'
                                        placeholder='Enter Manager Passcode'
                                        invalid={errors.passCode && true}
                                        {...field}
                                    />
                                )}
                            />
                            {errors.passCode && <span className='text-danger'>Passcode is required</span>}
                        </div>
                    </Col>
                </>
            )}
            ...
            <Col style={{ marginLeft: '30px' }} sm='12' className='mb-2'>
                <div className="mb-1" style={{ width: '400px' }}>
                    <Label className="form-label" htmlFor="reason" style={{ fontSize: '15px', paddingRight: '12px' }}>
                        Select Reason
                    </Label>
                    <Controller
                        id="reason"
            ...
            <div align='end' className='buttons'>
                        <Button outline className='me-1' color='secondary' onClick={handleCancelItemClick}>
                            CANCEL
                        </Button>
                        <Button color='primary' className='me-1' type="submit" disabled={isButtonClicked}>
                        {isButtonClicked ? 'Processing...' : 'Submit'}
            ...
            {/* <Row>
                <Col sm="12" md="6" className="mb-2" style={{ paddingLeft: '30px' }}>
                    <Button outline className="bg-transparent" style={{ width: "100%", backgroundColor: "gray" }} onClick={handleCancelItemClick}>
                        CANCEL
                    </Button>
                </Col>
                <Col sm="12" md="6" className="mb-2" style={{ paddingRight: '30px' }}>
            ...
            >
                <ModalBody className='px-5 pb-2'>
                    ...
                    <div className="button-container text-center">
                        ...
                        <Button className="me-1"  color="danger" onClick={() => setbillInfoModal(false)}>
                            No
                        </Button>

                        <Button color="primary" className='bg-transparent' onClick={handlePaymentPage}>
            ...
            <Modal isOpen={kotConfirmPopup} 
            // toggle={() => setkotConfirmPopup(false)} 
            className="modal-dialog-centered" style={{ width: '25%' }}>
                <ModalHeader 
                // toggle={() => setkotConfirmPopup(false)}
                ></ModalHeader>
                ...
                <ModalBody>
                    <Form onSubmit={handleSubmit(handleInvForm)}>
                        ...
                        <div align='end' className='buttons'>
                            ...
                            <Button color='primary' className='me-1' onClick={handleInvForm} disabled={loading}>
                    ...
                    <Form>
                        <Row>
                            ...
                            <Col>
                                ...
                                <div align='end' className='buttons'>
                                    <Button outline className='me-1' color='secondary' onClick={handleonCancelmodify}>
                                        CANCEL
                                    </Button>
