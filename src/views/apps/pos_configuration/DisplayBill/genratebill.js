...
import { Card, CardBody, CardHeader, CardTitle, CardText, Label, InputGroup, Input, Row, Container, Col, Table, Button, Modal, ModalHeader, ModalFooter, ModalBody, Form, InputGroupText,FormGroup } from 'reactstrap';
...
const PreviewPayment = ({ data, print }) => {
  ...
  const [discountType, setDiscountType] = useState('amt'); // Initial state: 'amt' for amount
  ...
  const onSubmitDisc = (data) => {
    ...
    let res = fetchx(API_URL + '/applyDiscount', {
      ...
      .then(result => {
        ...
        setfinalBalanceAmt(result['data']['Balance'])
        settotalDiscount(result['data']['TotalDiscount'])
        setinvBalance1(result['data']['Balance'])
...
setRoomDiscount(true)
  ...
  return typeof (data) != 'undefined' ? (
    ...
    {selectedVoucher && (
       <CouponForm voucher={selectedVoucher}
