...
const Floor = ({ data1 }) => {
  ...
  const [stewardId, setStewardId] = useState(data1); 
  ...
  useEffect(() => {
    if (stewardId) {
      fetchx(`${API_URL}/getPOSTableListStewardwise?stewardID=${stewardId}&hotelID=10`, {
        method: "GET",
      })
  ...
  const handleBulkDelete = () => {
    ...
    const idsToDelete = selectedRows.map(row => row.tableNo); // Collect the ids of the selected rows
    ...
    if (idsToDelete.length > 0) {
      ...
      fetchx(API_URL + `/deletestewardTableMap`, {      
        method: "POST",
        body: updatedItem,
        ...
        .then((res) => {
          ...
          .then((rowData) => setRowData(rowData["data"]));
            ...
            text: "Removed Tables Mapped Successfully!",
            icon: "success",
