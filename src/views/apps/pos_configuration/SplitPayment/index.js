...
import CouponFormSplit from './couponFormsplit.js';
import RedemptionHistorySplit from './redemptionHistorysplit.js'

const MySwal = withReactContent(Swal)

// ** Reactstrap Imports
import { Row, Col, Alert, CardTitle, CardHeader } from 'reactstrap'
import { Form, Label, Card, CardBody, CardText, Container, Button, Input, InputGroup, Modal, ModalBody, ModalHeader, ModalFooter, Table ,FormGroup} from 'reactstrap'
const SplitPayment = () => {
  // ** HooksVars
  const { id } = useParams()
  ...
  const [selectedservicechargeFolio, setselectedservicechargeFolio] = useState(null);
      ...
      cardArray.push(
        ...
        >
          ...
          <CardBody style={{ textAlign: "center", color: "black", display: "flex", flexDirection: "column", alignItems: "center" }}>
            ...
            onClick={() => handleServiceCharge(Folio,cardData)}
          >
            {/* Service Charge */}
            {cardData['isServiceCharge'] ? 'Service Charge (Applied)' : 'Service Charge (Removed)'}
...
if(balanceValue != 0.00){
  ...
  const handleFinishOrder = () => {
    ...
    setTimeout(() => {
      ...
      cancelButton.style.marginRight = '10px'; // Adjust the margin as needed
  ...
  const handleCancelVoidBillClick = () => {
    ...
    const handleServiceCharge = (Folio,cardData) => {
      setselectedservicechargeFolio(Folio)
      setIsServiceCharge(cardData['isServiceCharge'])
      setservicechargeInfoModal(true)
    }

    const handleaddserviceCharge = () => {
      console.log(`Adding service charge for Folio: ${selectedservicechargeFolio}`);
      ...
      const addservicechargedata = JSON.stringify({
       ...
       "folioNo": parseInt(selectedservicechargeFolio),
      });
      console.log("addservicechargedata=====================")
      console.log(addservicechargedata)
    ...
    const handleremoveserviceCharge = () =>{
      console.log(`Removing service charge for Folio: ${selectedservicechargeFolio}`);
      ...
      console.log("removeservicechargedata=====================")
      console.log(removeservicechargedata)
  ...
  <h4 className="m-0">
      ...
      >
        ...
        <Card className='invoice-preview-card'>
          <CardBody className='invoice-padding pb-0'>
            <div className='d-flex justify-content-between flex-md-row flex-column invoice-spacing mt-0'>
              <div>
                ...
                <div style={{paddingTop:'5px'}}>
                   {/* <img style={{   width: '80px',   height: '80px',   display: 'block',   marginBottom: '20px' }} src={`https://testpms.ms-tech.in/v8/pms-backend-prod/${logoimage}`} alt="Hotel Logo" onError={(e) => {   console.log('Error loading image:', e); }}/> */}
                   <a href="https://www.theoterra.com/net_zero_hotel" target="_blank" rel="noopener noreferrer">
                   <img style={{width: '80px',   height: '80px',   display: 'block',   marginBottom: '20px' }} src={API_URL+`/imagepaths/${logoimage}`} alt="Hotel Logo" onError={(e) => {   console.log('Error loading image:', e); }}/>
                </a>
                </div>
          ...
          <CardBody className='invoice-padding pb-0'>
            ...
            {console.log(finalInvBillinfo)}
            {finalInvBillinfo['membershipID'] && finalInvBillinfo['voucherCode'] && 
                <>
                    <h6 className='mb-0' style={{ display: 'inline', marginRight: '20px' }}>
                        Membership ID: {finalInvBillinfo['membershipID']}
                    </h6>
                    <h6 className='mb-0' style={{ display: 'inline' }}>
                        Voucher Code: {finalInvBillinfo['voucherCode']}
                    </h6>
                </>
            }
          </CardBody>
    ...
    <Col sm='2' style={{ display: 'flex', justifyContent: 'flex-end', alignItems: 'center' }}>
          ...
          <form onSubmit={handleSubmit(onSubmitDisc)}>
            ...
            <div align='end' className='buttons'>
              {/* <Button outline className='me-1' color='secondary' onClick={() => { setdiscountModal(!discountModal) }}>
               ...
               <Button outline className='me-1' color='secondary' onClick={handleCancelDiscountModal }>
                CANCEL
              </Button>
      ...
      <Modal isOpen={emailModal}
        ...
        <ModalBody className='px-5 pb-2'>
          <Form onSubmit={handleSubmit(confirmSubmit)}>
            ...
            <div className="button-container text-center">
              <Button className="me-1" color="danger" onClick={confirmSubmit1}>
                Cancel
              </Button>
              <Button className='bg-transparent' type='submit' color="primary">
                proceed
              </Button>
            </div>
          </Form>
      ...
      <div>
           {/* <CouponForm data={sessionStorage.getItem('VoucherData')} */}

    {selectedVoucher && (
       <CouponFormSplit voucher={selectedVoucher} selectedFolioNumber = {discFolio}
    ...
    <div>
       ...
       <RedemptionHistorySplit phoneNumberRedemption={phoneNumberRedemption}
