...
import { Modal, ModalHeader, ModalBody, ModalFooter, Button, Form, Row, Col, InputGroup, Input,Label} from 'reactstrap';
...
const CouponForm = ({ isOpen, onClose, voucher }) => {
  const { selectedVoucher, FolioNumber } = voucher;
  ...
  const membershipId = selectedVoucher.membershipId;
  const mappedVoucherId = selectedVoucher.mappedVoucherId;
  const voucherCode = selectedVoucher.voucherCode;
  const VoucherType = selectedVoucher.VoucherType;
  const voucherId = selectedVoucher.voucherId;
  const guestId = selectedVoucher.guestID;
  const quantity = selectedVoucher.quantity;
  const [amount, setAmount] = useState(''); 
  const [reason, setReason] = useState('');
  const [memeberShipIDInvoice, setmemeberShipIDInvoice] = useState(mappedVoucherId);

  const handleFormSubmit = (event) => {
    let passCode = event.target.elements.passCode.value;
    let managerID = event.target.elements.managerID.value;
    ...
    if (foodDisc === 0 && liquorDisc === 0 && softdrinksDisc === 0 && smokesDisc === 0 && othersDisc === 0) {
      MySwal.fire({
          text: "At least one percentage value is required",
              ...
              let res = fetchx(API_URL + '/applyDiscount', {
                  ...
                  .then(result => {  
                    ...
                    if (result['statuscode'] == 200) {
                      ...
                      swalInstance.then((result) => {
                        if (result.isConfirmed) {
                          // navigate('/apps/posconfiguration/DisplayBillPayment');
                          ...
                          return fetchx(API_URL + '/getPOSOrderDetails', {
                            method: 'POST',
                            body: billfetch,
                            headers: {
                              'Content-type': 'application/json; charset=UTF-8',
...
<Modal isOpen={isOpen} style={{ maxWidth: '650px' }}>
      ...
      <ModalBody>
        <Form onSubmit={handleFormSubmit}>
          ...
          <Col sm='12' className='mb-1' style={{marginBottom:'30px'}}>
                        <div className="mb-1">
