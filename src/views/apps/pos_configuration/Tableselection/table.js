    ...
    if (event.detail === 2) {
      fetchx(API_URL + '/getTableOrderStatus?hotelID=1&storeID=' + localStorage.getItem('storeID') + '&tableNo=' + count)
        ...
        .then(resp => {
          ...
          localStorage.setItem('orderID', tableOrderID);
          ...
          if (tableOrderID !== 0) {
            ...
            .then(resp => {
              if (resp['statuscode'] === 200 && resp['data'] === true) {
                ...
                return Promise.reject("Navigated to DisplayBillPayment due to split status.");
              }
              return tableOrderID; // Proceed to check payment status
            });
          } else {
            return Promise.reject(new Error("Invalid tableOrderID"));
          }
        })
        .then(tableOrderID => {
          return fetchx(API_URL + '/CheckPOSPaymentInitiated', {
            method: 'POST',
            body: JSON.stringify({
              "hotelID": 1,
              "storeID": localStorage.getItem('storeID'),
        ...
        .then(resp => {
          if (
            resp['statusCode'] === 403 &&
            resp['message'] === "Payment Initiated, Cannot perform this action"
          ) {
            navigate('/apps/posconfiguration/DisplayBillPayment');
          } else {
            return fetchx(API_URL + '/gettableorderlist?hotelID=1&storeID=' + localStorage.getItem('storeID') + '&tableNo=' + count + '&orderID=' + localStorage.getItem('orderID'))
              .then(result => result.json())
              .then(result => {
      ...
      fetchx(API_URL + '/getTableOrderStatus?hotelID=1&storeID=' + localStorage.getItem('storeID') + '&tableNo=' + count)
        ...
        .then(resp => {
          ...
          if (tableOrderID !== 0) {
            fetchx(API_URL + '/gettableorderlist?hotelID=1&storeID=' + localStorage.getItem('storeID') + '&tableNo=' + count + '&orderID=' + tableOrderID)
              .then(result => result.json())
              .then(result => {
                settableorders(result['data']);
                setreloadtable(true);
            ...
            <tr>
              ...
              <th style={{ width: '150px' }}>
                ...
                m
                    ...
                    {kot && Array.isArray(kot.items) && kot.items.map((item) => (
              <tr key={item.id}>
                <td style={{ maxWidth: '150px' }}>{item.itemName}</td>
                <td style={{ width: '90px' }}>Qty: {item.qty}</td>
              </tr>
