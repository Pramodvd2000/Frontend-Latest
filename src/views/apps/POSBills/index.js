...
const [FilterData,setFilterData] = useState([])
  ...
  const [columnDefs, setColumnDefs] = useState([
    ...
    {
      headerName: 'POS Invoice', cellRendererFramework: (params) => <Button color='primary' style={{ width: 80 }} onClick={() => {
        getFinalInvoice(params.data)


      }} >View</Button>, suppressSizeToFit: true, maxWidth: 130
    },
    ...
    <Button color='primary' style={{ width: 130 }} onClick={() =>{
      ...
      // 
    } } >Attachments</Button>)},maxWidth: 170 },
    {
    ...
    { headerName: 'Customer Name', field: 'guestName',maxWidth: 180 },
            { headerName: 'GuestGST', field: 'GuestGST',maxWidth: 120 },
    { headerName: 'CompanyGST', field: 'companyGST' },
...
const applyGSTInvoices = () =>{
  ...
  const filteredData = allData.filter(row => row.GuestGST?.trim() || row.companyGST?.trim());
    ...
    if (date) {
      ...
      fetchx(API_URL + '/getAllBillsByDate?hotelID=1&storeID=' + storeID + '&dateTime=' + date)
        ...
        .then(data => {
          ...
          // else {
            if (data['statuscode'] === 200 || data['statusCode'] === 200 || !data['data'] || !Array.isArray(data['data'])) {
    ...
    fetchx(API_URL + '/resettlePaymentMode', {
      ...
      .then((post) => {
        ...
        if (requestBody.FolioNo === 0 || requestBody.FolioNo === "0") {
          setTimeout(() => { navigate('/apps/posconfiguration/DisplayBillPayment') }, 100);
        } else {
          setTimeout(() => { navigate('/apps/posconfiguration/DisplayBillPayment') }, 100);
        }
      }
...
const SendEmail = (data) =>{
  ...
  const getRowStyle = params => {
    ...
    if (params.data && ((params.data.GuestGST != "" && params.data.GuestGST != null)  ||  (params.data.companyGST != "" && params.data.companyGST != null))) {
        ...
        <ModalBody className='pb-3 px-sm-1 mx-20' >
          ...
          <Form onSubmit={handleSubmit(onSubmit)}>
            ...
            <div align='end' className='buttons'>
              <Button outline className='me-1' color='secondary' onClick={handleCancelConfirmationModal}>
                CANCEL
              </Button>
