...
import { Card, Form, Label, Button, CardBody, CardTitle, CardHeader, Row, Col, Modal, ModalHeader, ModalBody,} from "reactstrap";
...
const Extras = (data1) => {
  ...
  const onSubmit = (data) => {
    ...
    let res = fetchx(API_URL2 + "/addRateCodeExtas", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: createExtra,
    }).then((res) => {
      if (res["status"] == 200) {
        fetchx(API_URL2 +`/getreservationRateCodeExtra?hotelID=1&rateCodeID=${data1.data1.id}`)
          .then((result) => result.json())
          .then((rowData) => {
            setRowData(rowData["data"]);
            //console.log(rowData["data"]);
...
// //console.log(completeData)
  function Confirm() {
    ...
    extraID:completeData.extraID,
     ...
     fetchx(API_URL2 + `/deleteRateCodeExtras`, {      
    method: "POST",
    body: updatedItem,
    headers: { "Content-type": "application/json" },
  })
