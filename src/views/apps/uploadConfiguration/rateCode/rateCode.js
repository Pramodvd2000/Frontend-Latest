...
let packages = [
  fetchx(API_URL2 + '/getRateCodePackageID?hotelID=1 ')
...
let sources = [
  fetchx(API_URL2 + '/getRateCodeSourceID?hotelID=1 ')
    .then(result => result.json())
    .then(resp => {
      sources = resp['data']
    })
...
let marketCodes = [
  fetchx(API_URL2 + '/getRateCodeMarketID?hotelID=1 ')
    .then(result => result.json())
    .then(resp => {
...
let rateCategory = [
  fetchx(API_URL2 + '/getRateCodeRateCategory?hotelID=1')
    .then(result => result.json())
    .then(resp => {
...
const rateCode = ({stepper}) => {
  ...
  useEffect(() => {
    ...
    fetchx(API_URL2 +"/getCompleteAccountDetails?hotelID=1")
      .then((result) => result.json())
...
const onFilterTextBoxChanged2 = useCallback(() => {
  ...
  const packagetransactionCode = (selectedPackage) => {
    fetchx(API_URL2 + `/getPackageTransactionCode?id=${selectedPackage.value}&hotelID=1`)
    .then(result => result.json())
    .then(resp => {
  ...
  const onSubmit = data => {
    setData(data)
      ...
      let res = fetchx(API_URL2+"/addRateCodeDetails", {
        method: "PUT",
