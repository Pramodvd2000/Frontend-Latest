...
const RateCodeRoomRates = (data1) => {
  ...
  const [Rate2, setRate2] = useState(); // fetchx(API_URL2 + `/getRoomtypes?hotelID=1`)
  ...
  const cellClickedListener1 = (event) => {
    ...
    setRateCode(false);
    fetchx(API_URL2 + `/getAllRoomtypeDataForRateCode?hotelID=1&rateCodeID=${data1.data1["id"]}`)
    .then((result) => result.json())
    .then((rowData) => setRowData(rowData["data"]));
    // Confirm1()
  };
  ...
  const onSubmit = (data) => {
      ...
      let res = fetchx(API_URL2 +"/updateRateCodeRoomRateDetails", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: createrateCode,
      })
