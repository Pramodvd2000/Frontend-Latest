...
const RateCodeRoomRates = ({ stepper2,data1}) => {
  ...
  useEffect(() => {
    fetchx(API_URL2 + "/getroomtyperates", {
      ...
      fetchx(API_URL2+`/getRemainingRoomtypeData?hotelID=1&rateCodeID=${data1.id}`)
      .then((result) => result.json())
  ...
  function Confirm1() {
    ...
    hotelID:1,   
     ...
     fetchx(API_URL2 + `/deactivateRateCodeRoomRates`, {      
    method: "PUT",
    body: updatedItem,
    headers: { "Content-type": "application/json" },
  })
  ...
  // useEffect(() => {
  //   fetchx(API_URL2+`/getRoomtypes?hotelID=1`)
  //     .then((result) => result.json())
  //     .then((rowData) => setRowData(rowData["data"]));
  //  console.log(rowData);
  //   }, []);
