...
const Companydetails = (data1) => {
  //console.logdata1);
    ...
    const onSubmit = (data) => {
        ...
        if (data.accountType !== null && data.accountName !== null) {
          ...
          //console.log"hi");
          let res = fetchx(API_URL2+"/addAccountRateCodeMapping", {
            method: "POST",
    ...
    useEffect(() => {
      fetchx(API_URL2 + `/getCompanyList?hotelID=1`)
        .then(result => result.json())
        .then(rowData => {
  ...
  const AccountMapping = ({stepper2,data1}) => {
    ...
    useEffect(() => {
      ...
      fetchx(API_URL2 + `/getRatecodeAccount?hotelID=1&rateCodeID=${data1.id}}`)
        .then((result) => result.json())
  ...
  function Confirm() {
      ...
      rateCodeID: completeData.rateCodeID,      
       ...
       fetchx(API_URL2 + `/deleteAccontRateCodeMap`, {      
      method: "POST",
      body: updatedItem,
      headers: { "Content-type": "application/json" },
    })
      ...
      .then((post) => {
          ...
          setPopUp(false);
          fetchx(API_URL2 + `/getAccRatecode?hotelID=1&accountID=${data1.companyid}`)
            .then((result) => result.json())
            .then((rowData) => {
            });
        }
