...
const RateCodeRoomRates = ({ stepper2,data1}) => {
 ////console.log(data1.data1)
  ...
  useEffect(() => {
    ...
    const fetchRoomtype = async () => {
      try {
        const response = await fetchx(API_URL2 + `/getRemainingRoomTypesRates?rateCodeID=${data1.id}`);
 ...
 ////console.log(roomtype)
  ...
  let colourMappings = [
    fetchx(API_URL2 + "/getExtraDescription?hotelID=1")
  ...
  const onSubmit = data => {
   ...
   ////console.log(data)
    if (
     ...
     ////console.log("hi")
      let res = fetchx(API_URL2 + "/addrateCodeRoomRate", {
        method: "POST",
       ...
       ////console.log(res);
        if (res['status'] == 200) {
          fetchx(API_URL2 +`/getAccRatecodeRoomRate?id=${data1.id}`)
            .then(result => result.json())
            .then(rowData => {
             ...
             ////console.log(rowData['data'])
              ...
              swalInstance.then((result) => {
                if (result.isConfirmed) {
                  ...
                  // useEffect();
                  setRoomRatess(false);
                  // fetchx(API_URL2 +`/getRemainingRoomTypesRates?rateCodeID=${data1.id}`)
                  fetchx(API_URL2 +`/getAccRatecodeRoomRate?id=${data1.id}`)
                  .then((result) => result.json())
...
//console.log(data1.id)
    ...
    const actionButton1 = (rowval) => {
     ...
     ////console.log(createmarketGroup)
      ...
      let res = fetchx(API_URL2+`/updateRateCodeRoomRates`,
      {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: createmarketGroup,
...
//console.log(localStorage.getItem("ratedata"));
  ...
  useEffect(() => {
    fetchx(API_URL2 + "/getroomtyperates", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        hotelID: "1",
 ...
 ////console.log(rowData);
  ...
  function Confirm() {
    ...
    roomTypeID:completeData.roomTypeID,
     ...
     fetchx(API_URL2 + `/deleteRateCodeRoomRates`, {      
    method: "POST",
    body: updatedItem,
    headers: { "Content-type": "application/json" },
  })
