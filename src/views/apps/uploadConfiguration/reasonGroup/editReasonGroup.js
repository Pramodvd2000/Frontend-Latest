  ...
  const onSubmit = (data) => {
    setData(data);
      let createmarketGroup = JSON.stringify({
        "id": data1.id,
        "reasonGroup": data.reasonGroup1,
        "description":  data.description1,
     
          }); 
      let columnsToUpdate = createmarketGroup
      let res = fetchx(API_URL2 + `/updateReasonGroup?id=${data1.id}`,
        {
          method: "PUT",
        ...
        .then((resp) => {
            ...
            const swalInstance = MySwal.fire({
             text: 'Reason Group Details Updated Successfully!',
             icon: 'success',
         ...
         swalInstance.then((result) => {
           if (result.isConfirmed) {      
            ...
            const swalInstance = MySwal.fire({
                text: 'Reason Group Details Not Updated!',
                icon: 'error',
                buttonsStyling: false,
                confirmButtonText: 'Close',
                customClass: {
  ...
  return (
    <div>
        <Form onSubmit={handleSubmit(onSubmit)}>
            ...
            <Col md="3" sm="12">
                <div className="mb-1">
                  <Label className="form-label" for="reasonGroup1">
                  Reason Group <spam style={{ color: "red" }}>*</spam>
                  </Label>
                  <Controller
                    defaultValue={data1["reasonGroup"]}
                    control={control}
                    id="reasonGroup1"
                    name="reasonGroup1"
                    render={({ field }) => (
                      <Input
                        required
                        placeholder="Name"
                        invalid={errors.reasonGroup1 && true}
                        {...field}
                      />
                    )}
                  />
