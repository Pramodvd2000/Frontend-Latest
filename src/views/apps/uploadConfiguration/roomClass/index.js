...
import EditMarketGroup from "./editRoomClass"
...
const SourceCode = () => {
  ...
  const onSubmit = data => {
    ...
    if ( data.marketCode!==null && data.marketGroupID!==null) {
      ...
      let res = fetchx(API_URL2 +"/addroomclass", {
        method: "POST",
        headers: { 'Content-Type': 'application/json' },
  ...
  return (
    <div>
     ...
     <div>
      ...
      {hotelLive === 0 && (<Card>
      <CardHeader><h4><b> Add Room Class</b></h4></CardHeader>
     </Card>)}
      {hotelLive === 0 && ( <p style={{color: "grey"}}> 
      <b><h4> Instructions:  </h4> </b>
      1.Please refer to the sample file before uploading the data. You can access the sample file by clicking the  <b>"Room Class File" </b> button. Please ensure that data insertion follows the established pattern.<br/> Kindly refrain from altering the file structure.<br/>
