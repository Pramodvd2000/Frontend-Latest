...
import EditMarketGroup from "./editPackage"
...
let packages = [
  fetchx(API_URL2 +"/getGroupForPackage?hotelID=1 ")
...
let transactionCode = [
  fetchx(API_URL2 +"/getRateCodeTransactionID?hotelID=1")
    .then((result) => result.json())
    .then((resp) => {
      // console.log(resp['data'])
...
// console.log(transactionData)
  ...
  const updatedItem = JSON.stringify({            
    ...
    //  console.log(updatedItem)
     fetchx(API_URL2+ `/updateConPackage?id=${event.data.id}`, {
     method: 'PUT',
     body: updatedItem,
     headers: {
...
// console.log(updatedItem)
fetchx(API_URL2+ `/updateConPackage`, {
method: 'PUT',
body: updatedItem,
headers: {
'Content-type': 'application/json'
...
const [hotelLive, sethotelLive] = useState(null);
  ...
  console.log(rowData)
  useEffect(() => {
    fetchx(API_URL2 +"/gettransactioncode?hotelID=1")
      .then((result) => result.json())
    ...
    if (data.rateCode !== null && data.transactionCode !== null && data.beginSellDate !== null)
     {
      ...
      let res = fetchx(API_URL2 + "/addpackage", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: createrateCode,
      }).then(data => data.json())
      .then((res) => {
        ...
        if (res["statusCode"] == 200) {
          fetchx(API_URL2 +"/getPackage?hotelID=?")
            .then((result) => result.json())
            .then((rowData) => {
              setRowData(rowData["data"]);
              console.log(rowData["data"]);
...
<div>
      ...
      {hotelLive === 0 && (<Card>
      <CardHeader><h4><b> Add Package</b></h4></CardHeader>
     </Card>)}
      {hotelLive === 0 && ( <p style={{color: "grey"}}> 
      <b><h4> Instructions:  </h4> </b>
      1.Please refer to the sample file before uploading the data. You can access the sample file by clicking the  <b>"Package File" </b> button. Please ensure that data insertion follows the established pattern.<br/> Kindly refrain from altering the file structure.<br/>
...
<div>
      ...
      <div align="end" className="buttons">
        {/* <Button className='me-1' outline color='secondary' onClick={() => handleDownload('origin.csv')}>Package File</Button> */}
