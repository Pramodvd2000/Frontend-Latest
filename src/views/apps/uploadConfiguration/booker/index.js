...
import { UncontrolledAccordion,Accordion, AccordionBody, AccordionHeader, AccordionItem,Modal,
  ModalHeader,
  ModalBody,
...
import EditMarketGroup from "./editBlock"
...
// ];
let countryoptions = [
  fetchx(API_URL2+'/getGuestProfileCountry?hotelID=1')
...
const ValidationThirdPartyComponents = ({data1}) => {
  ...
  const updatedItem = JSON.stringify({            
    ...
    //  console.log(updatedItem)
     fetchx(API_URL2+ `/updateBookerDetails`, {
     method: 'PUT',
     body: updatedItem,
     headers: {
...
console.log("oldValue",oldValue)
  ...
  const onSubmit = (data) => {
    ...
    // console.log(data)
   {
      ...
      let res = fetchx(API_URL2+"/addbooker", {
        method: "POST",
        headers: { 'Content-Type': 'application/json' },
        body: createasset
      }).then(data => data.json())
      .then((res) => {
        ...
        if(res['statusCode']==200){
          fetchx( API_URL2 +  '/getbooker?hotelID='+id)
          .then(result => result.json())
          .then(rowData => {setRowData(rowData['data'])
          // console.log(rowData['data'])
          const swalInstance = MySwal.fire({
 ...
 {hotelLive === 0 && (<Card>
      <CardHeader><h4><b> Add Booker</b></h4></CardHeader>
     </Card>)}
      {hotelLive === 0 && ( <p style={{color: "grey"}}> 
      <b><h4> Instructions:  </h4> </b>
      1.Please refer to the sample file before uploading the data. You can access the sample file by clicking the  <b>"Booker File" </b> button. Please ensure that data insertion follows the established pattern.<br/> Kindly refrain from altering the file structure.<br/>
