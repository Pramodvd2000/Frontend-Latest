...
import { Card, Form, Row, Col, Label, Button, CardBody, Input, InputGroup, CardHeader,  } from 'reactstrap'
...
import { UncontrolledAccordion, AccordionBody, AccordionHeader, AccordionItem,Modal,ModalBody,ModalHeader } from 'reactstrap'
...
const Block = () =>{
  ...
  useEffect(() => {
    ...
    fetchx(API_URL2 +'/getBlock?hotelID=1')
  ...
  const onSubmit = data => {
    ...
    if ( data.block.length) {
      ...
      let res = fetchx(API_URL2 + "/addBlock", {
        method: "POST",
        headers: { 'Content-Type': 'application/json' },
  ...
  return (
   ...
   <div>
      ...
      {hotelLive === 0 && (<Card>
      <CardHeader><h4><b> Add Block</b></h4
      ></CardHeader>
     </Card>)}
      {hotelLive === 0 && ( <p style={{color: "grey"}}> 
      <b><h4> Instructions:  </h4> </b>
      1.Please refer to the sample file before uploading the data. You can access the sample file by clicking the  <b>"Block File" </b> button. Please ensure that data insertion follows the established pattern.<br/> Kindly refrain from altering the file structure.<br/>
