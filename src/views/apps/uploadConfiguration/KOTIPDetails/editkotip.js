...
const RestaurantDetails = (data1) => {
    ...
    const onSubmit = (data) => {
     ...
     data.KOTprint = checkboxChecked1 ? "Enable" : "Disable";
        ...
        const query = 'UPDATE KitchenIPDetails SET hotelID=?,storeID=?,IP=?,menuHead=?,menuGroup=?,status=? WHERE id=?';
        ...
        let createStoreData = JSON.stringify({
            ...
            menuGroup: data1.data1.menuGroup,
            id: data1.data1.id
        });

        let res = fetchx(API_URL2 + `/updateKitchenIPDetails`, {
            method: "PUT",
            ...
            .then((resp) => {
                if (resp['statusCode'] == 200) {
                    ...
                    const swalInstance = MySwal.fire({
                        text: 'KOT IP Details Edited Successfully!',
                        icon: 'success',
                    ...
                    <Col md="4" sm="12">
                        <div className="mb-1">
                            <Label className="form-label" for="IP">
                                IP <spam style={{ color: "red" }}>*</spam>
                            </Label>
                            <Controller
                                defaultValue={data1.data1.IP}
                                control={control}
                                id="IP"
                                name="IP"
                                render={({ field }) => (
