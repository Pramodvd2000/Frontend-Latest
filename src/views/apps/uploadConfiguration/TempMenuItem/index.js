...
fetchx(API_URL2+'/getmenugroupOptions?hotelID=1&storeID=1')
.then(result => result.json())
.then(rowData => {
...
const ValidationThirdPartyComponents = () => {
  ...
  useEffect(() => {
    fetchx(API_URL2 + '/getTempMenuItems')
      .then(result => result.json())
      .then(rowData => {
  ...
  useEffect(() => {
    ...
    fetchx(API_URL2 + "/getBusinessDatePOS", {
      method: "POST",
  ...
  const handleSelectChange = (event) => {
    if (event) {
      ...
      fetchx(API_URL2 + '/getmenugroupOptions?hotelID=1&storeID=' + event.value)
        .then(result => result.json())
        .then(rowData => {
          menuGroupOptions = rowData['data']
        })
  ...
  const onSubmit = (data) => {
    ...
    if (
      ...
      let res = fetchx(API_URL2 + "/addtempmenuitem", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: menuitem,
      })
      ...
      .then((res) => {
        ...
        if (res['statuscode'] == 200) {
          ...
          swalInstance.then((result) => {
            ...
            fetchx(API_URL2+'/getTempMenuItems?hotelID=1')
            .then(result => result.json())
