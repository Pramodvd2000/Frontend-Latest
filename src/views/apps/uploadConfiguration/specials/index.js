...
import EditGuestpreference from "./editSpecials"
...
let specialGroupID = [
  fetchx(API_URL2 +'/getSpecialGroupID?hotelID=1')
...
const Specials = () => {
  ...
  const updatedItem = JSON.stringify({                  
     ...
     fetchx(API_URL2+`/updateSpecials?id=${event.data.id}`, {
     method: 'PUT',
     body: updatedItem,
     headers: {
...
console.log(updatedItem)
fetchx(API_URL2+ `/updateSpecials`, {
method: 'PUT',
body: updatedItem,
headers: {
'Content-type': 'application/json'
...
useEffect(() => {
  ...
  const onSubmit = data => {
    ...
    if (data.preference.length && data.description.length) {
        ...
        let res = fetchx(API_URL2 + "/addSpecials", {
          method: "POST",
          headers: { 'Content-Type': 'application/json' },
          body: createspecial
        }).then(data => data.json())
...
<div>
        <Modal  isOpen={marketGroup} toggle={() => setMarketGroup(!marketGroup)}className="modal-lg"  >
          <ModalHeader className="modal-lg"toggle={() => setMarketGroup(!marketGroup)} >
           Edit Guest Preference Details
          </ModalHeader>
          <ModalBody className="pb-3 px-sm-1 mx-20">            
            <EditGuestpreference data1={filldata} />
          </ModalBody>
        </Modal>
      </div>
      
{hotelLive === 0 && (<Card>
      <CardHeader><h4><b> Add Preferences</b></h4></CardHeader>
     </Card>)}
      {hotelLive === 0 && ( <p style={{color: "grey"}}> 
      <b><h4> Instructions:  </h4> </b>
      1.Please refer to the sample file before uploading the data. You can access the sample file by clicking the  <b>"Preferences File" </b> button. Please ensure that data insertion follows the established pattern.<br/> Kindly refrain from altering the file structure.<br/>
