  ...
  const onSubmit = (data) => {
    setData(data);
      let createmarketGroup = JSON.stringify({
        "id": data1.id,
        "sourceGroup": data.sourceGroup1,
        "description":  data.description1,
     
          }); 
      let columnsToUpdate = createmarketGroup
      let res = fetchx(API_URL2 + `/updateSourceGroups?id=${data1.id}`,
        {
          method: "PUT",
        ...
        .then((resp) => {
            ...
            const swalInstance = MySwal.fire({
             text: 'Source Group Details Updated Successfully!',
             icon: 'success',
         ...
         swalInstance.then((result) => {
           if (result.isConfirmed) {      
            ...
            const swalInstance = MySwal.fire({
                text: 'Source Group Details Not Updated!',
                icon: 'error',
                buttonsStyling: false,
                confirmButtonText: 'Close',
                customClass: {
  ...
  return (
    <div>
        <Form onSubmit={handleSubmit(onSubmit)}>
            ...
            <Col md="3" sm="12">
                <div className="mb-1">
                  <Label className="form-label" for="sourceGroup1">
                  Source Group <spam style={{ color: "red" }}>*</spam>
                  </Label>
                  <Controller
                    defaultValue={data1["sourceGroup"]}
                    control={control}
                    id="sourceGroup1"
                    name="sourceGroup1"
                    render={({ field }) => (
                      <Input
                        required
                        placeholder="Name"
                        invalid={errors.sourceGroup1 && true}
                        {...field}
                      />
                    )}
                  />
