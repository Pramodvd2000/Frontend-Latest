...
const id = '1';
import EditMarketGroup from "./editReason"
...
const Extras = () => {
  ...
  useEffect(() => {
    ...
    fetchx(API_URL2 + "/getBusinessDate", {
      ...
      .then(result => result.json())
      .then(rowData => {setreasonGroupData(rowData['data'])
      // console.log(rowData['data'])
      })

    fetchx(API_URL2 + '/getAllReasons?hotelID=1')
    .then(result => result.json())
  ...
  //     let res = fetchx(API_URL2 + "/addReasonCode", {
  //       method: "POST",
  //       headers: { "Content-Type": "application/json" },
  //       body: createExtra,
  //     }).then((res) => {
    ...
    .then((res) => {
      ...
      if (res.status === 200) {
        const swalInstance = MySwal.fire({
          text: 'Reasoncode Added Successfully!',
          icon: 'success',
        ...
        fetchx(API_URL2 + '/getAllReasons?hotelID=' + id)
          .then(result => result.json())
          .then(rowData => {
  ...
  return (
   ...
   <div>
      ...
      { reasonGroupData.length === 0 && <div style={{ color: 'red' }}>
      <h4 style={{ color: 'red' }}> <b> Please Upload Reason Group Data to the System first, only then you can enter Reason Code Details</b> </h4>
      </div>}

       {(hotelLive === 0 && reasonGroupData.length !== 0) && (<Card>
      <CardHeader><h4><b> Add Reason Code</b></h4></CardHeader>
     </Card>)}
      {(hotelLive === 0 && reasonGroupData.length !== 0) && ( <p style={{color: "grey"}}> 
      <b><h4> Instructions:  </h4> </b>
      1.Please refer to the sample file before uploading the data. You can access the sample file by clicking the  <b>"Reason Code File" </b> button. Please ensure that data insertion follows the established pattern.<br/> Kindly refrain from altering the file structure.<br/>
    ...
    {(hotelLive === 0 && reasonGroupData.length !== 0) && (
        <div> 
          <br/>
          <UploadDocs/>
      </div>)}
...
<div>
        <Row>
        {reasonGroupData.length !== 0 &&(<Col md="3" sm="12" className="mb-1">
    ...
    <div>
    <Card>
    {reasonGroupData.length !== 0 &&(<div className="ag-theme-alpine" style={{ height: 540}}>
        <AgGridReact 
            ref={gridRef}
