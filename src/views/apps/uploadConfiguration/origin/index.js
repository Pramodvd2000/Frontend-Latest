...
import { Card, Form, Row, Col, Label, Button, CardBody,Input, InputGroup,CardHeader,Modal,ModalHeader,ModalBody  } from 'reactstrap'
...
import EditMarketGroup from "./editOrigin"
...
const Origin = () => {  
  ...
  useEffect(() => {
    ...
    fetchx(API_URL2 +'/getOrigin?hotelID=10')
  ...
  const EditData = (rowData)=>{
   ...
   const [columnDefs, setColumnDefs] = useState([
    ...
    {headerName: 'Origin ',field: 'origin',suppressSizeToFit: true,cellStyle: {'textAlign': 'center','backgroundColor': '#F1E39B'}, maxWidth: 200 },
    {
      headerName: "Action",
      field: "numAvlRooms",
  ...
  const onSubmit = data => {
    setData(data)
      ...
      let res = fetchx(API_URL2 + "/addOrigin", {
        method: "POST",
        headers: { 'Content-Type': 'application/json' },
      ...
      .then((res) => {
        ...
        if(res['statusCode']==200){
          ...
          const swalInstance = MySwal.fire({
            text: 'Origin Added Successfully!',
            icon: 'success',
...
<div>
      ...
      {hotelLive === 0 && (<Card>
      <CardHeader><h4><b> Add origin</b></h4></CardHeader>
     </Card>)}
      {hotelLive === 0 && ( <p style={{color: "grey"}}> 
      <b><h4> Instructions:  </h4> </b>
      1.Please refer to the sample file before uploading the data. You can access the sample file by clicking the  <b>"Origin File" </b> button. Please ensure that data insertion follows the established pattern.<br/> Kindly refrain from altering the file structure.<br/>
    ...
    { hotelLive === 1 && (  <UncontrolledAccordion>
      <AccordionItem>
        <AccordionHeader targetId='1'><h4><b> Add Origin</b></h4></AccordionHeader>
