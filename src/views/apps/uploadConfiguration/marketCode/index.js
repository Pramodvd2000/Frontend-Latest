...
import EditMarketCode from "./editmarketCode"
...
let marketGroupID = [
  fetchx(API_URL2 +'/getmarketcodemarketgroupid?hotelID=1')
  ...
  const EditData = (rowData)=>{
    ...
    const updatedItem = JSON.stringify({            
      ...
      //  console.log(updatedItem)
       fetchx(API_URL2+`/updateMarketCodes?id=${event.data.id}`, {
       method: 'PUT',
       body: updatedItem,
       headers: {
...
// console.log(updatedItem)
fetchx(API_URL2+ `/updateMarketCodes`, {
method: 'PUT',
body: updatedItem,
headers: {
'Content-type': 'application/json'
...
setPopUp(false)
fetchx(API_URL2 + '/getMarketCode?hotelID=1')
.then(result => result.json())
.then(rowData =>{
...
const [oldValue, setOldValue] = useState(null);
  ...
  const onSubmit = data => {
    ...
    if ( data.marketCode!==null && data.marketGroupID!==null) {
      ...
      let res = fetchx(API_URL2 +'/addMarketCode', {
        method: "POST",
        headers: { 'Content-Type': 'application/json' },
     ...
     <div>
        <Modal  isOpen={marketGroup} toggle={() => setMarketGroup(!marketGroup)}className="modal-lg"  >
          <ModalHeader className="modal-lg"toggle={() => setMarketGroup(!marketGroup)} >
           Edit Market Code Details
          </ModalHeader>
          <ModalBody className="pb-3 px-sm-1 mx-20">            
            <EditMarketCode data1={filldata} />
          </ModalBody>
        </Modal>
      </div>

      { marketGroupData.length === 0 && <div style={{ color: 'red' }}>
      <h4 style={{ color: 'red' }}> <b> Please Upload Market Group Data to the System first, only then you can enter Market Code Details</b> </h4>
      </div>}

      {(hotelLive === 0  && marketGroupData.length !== 0)&& (<Card>
      <CardHeader><h4><b> Add Market Code</b></h4></CardHeader>
     </Card>)}
      {(hotelLive === 0  && marketGroupData.length !== 0)&& ( <p style={{color: "grey"}}> 
      <b><h4> Instructions:  </h4> </b>
      1.Please refer to the sample file before uploading the data. You can access the sample file by clicking the  <b>"Market Code File" </b> button. Please ensure that data insertion follows the established pattern.<br/> Kindly refrain from altering the file structure.<br/>
