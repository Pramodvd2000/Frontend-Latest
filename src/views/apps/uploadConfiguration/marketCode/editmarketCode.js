...
let marketGroupID = [
  ...
  const defaultReason4 = {
    value: data1.marketGroupID,
    label: data1.marketGroup,
  }; 
  ...
  const onSubmit = (data) => {
    setData(data);
      let createmarketGroup = JSON.stringify({
        "id": data1.id,
        "marketCode": data.marketCode1,
        "description":  data.description1,
        marketGroupID: (data.marketGroupID1 === undefined)
        ? data1.marketGroupID
        : (data.marketGroupID1 === null)
          ? null
          : data.marketGroupID1.value,
          }); 
      let columnsToUpdate = createmarketGroup
      let res = fetchx(API_URL2 + `/updatemarketcode?id=${data1.id}`,
        {
          method: "PUT",
        ...
        .then((resp) => {
            ...
            const swalInstance = MySwal.fire({
             text: 'Market Code Details Updated Successfully!',
             icon: 'success',
         ...
         swalInstance.then((result) => {
           if (result.isConfirmed) {      
            ...
            const swalInstance = MySwal.fire({
                text: 'Market Code Details Not Updated!',
                icon: 'error',
                buttonsStyling: false,
                confirmButtonText: 'Close',
                customClass: {
  ...
  return (
    <div>
        <Form onSubmit={handleSubmit(onSubmit)}>
                  ...
                  <Controller
                    ...
                    render={({ field }) => (
                      <Input
                        required
                        placeholder="Name"
                        invalid={errors.marketCode1 && true}
                        {...field}
                      />
                    )}
                  />
            ...
            <Col md="3" sm="12">
                <div className="mb-1">
                  <Label className="form-label" for="marketGroupID1">
                  Market Group
                  </Label>
                  <Controller
                    id="marketGroupID1"
