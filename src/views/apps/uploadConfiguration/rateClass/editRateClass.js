  ...
  const onSubmit = (data) => {
    setData(data);
      let createmarketGroup = JSON.stringify({
        "id": data1.id,
        "rateClass": data.rateClass1,
        "description":  data.description1,
     
          }); 
      let columnsToUpdate = createmarketGroup
      let res = fetchx(API_URL2 + `/updateRateClassData?id=${data1.id}`,
        {
          method: "PUT",
        ...
        .then((resp) => {
            ...
            const swalInstance = MySwal.fire({
             text: 'Rate Class Details Updated Successfully!',
             icon: 'success',
         ...
         swalInstance.then((result) => {
           if (result.isConfirmed) {      
            ...
            const swalInstance = MySwal.fire({
                text: 'Rate Class Details Not Updated!',
                icon: 'error',
                buttonsStyling: false,
                confirmButtonText: 'Close',
                customClass: {
  ...
  return (
    <div>
        <Form onSubmit={handleSubmit(onSubmit)}>
            ...
            <Col md="3" sm="12">
                <div className="mb-1">
                  <Label className="form-label" for="rateClass1">
                  Rate Class 
                  </Label>
                  <Controller
                    defaultValue={data1["rateClass"]}
                    control={control}
                    id="rateClass1"
                    name="rateClass1"
                    render={({ field }) => (
                      <Input
                        required
                        placeholder="Name"
                        invalid={errors.rateClass1 && true}
                        {...field}
                      />
                    )}
                  />
