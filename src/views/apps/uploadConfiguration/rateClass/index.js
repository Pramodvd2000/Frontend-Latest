 ...
 const [open, setOpen] = useState('')
  ...
  const updatedItem = JSON.stringify({            
    ...
    //  console.log(updatedItem)
     fetchx(API_URL2+`/updateRateClass?id=${event.data.id}`, {
     method: 'PUT',
     body: updatedItem,
     headers: {
...
// console.log(updatedItem)
fetchx(API_URL2+ `/updateRateClass`, {
method: 'PUT',
body: updatedItem,
headers: {
'Content-type': 'application/json'
...
setPopUp(false)
fetchx(API_URL2 + '/getRateClass?hotelID=1')
...
// console.log("oldValue",oldValue)
  ...
  const onSubmit = data => {
    ...
    if (data.rateClass!== null && data.description !== null ) {
        ...
        let res = fetchx(API_URL2 +"/addRateClass", {
          method: "POST",
          headers: { 'Content-Type': 'application/json' },
          body: createasset
        }).then(data => data.json())
...
<div>
        <Modal  isOpen={marketGroup} toggle={() => setMarketGroup(!marketGroup)}className="modal-lg"  >
          <ModalHeader className="modal-lg"toggle={() => setMarketGroup(!marketGroup)} >
           Edit RateClass Details
          </ModalHeader>
          <ModalBody className="pb-3 px-sm-1 mx-20">            
            <EditRateClass data1={filldata} />
          </ModalBody>
        </Modal>
      </div>
      {hotelLive === 0 && (<Card>
      <CardHeader><h4><b> Add RateClass</b></h4></CardHeader>
     </Card>)}
      {hotelLive === 0 && ( <p style={{color: "grey"}}> 
      <b><h4> Instructions:  </h4> </b>
      1.Please refer to the sample file before uploading the data. You can access the sample file by clicking the  <b>"RateClass File" </b> button. Please ensure that data insertion follows the established pattern.<br/> Kindly refrain from altering the file structure.<br/>
