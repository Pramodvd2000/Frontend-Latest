...
import {Card,CardHeader,Form,Label,Button,CardBody,Row,Col,InputGroup,Input,Modal,ModalHeader,ModalBody,} from "reactstrap";
...
import EditpreferenceGroup from "./editgroup"
...
const Preference = () => {
  ...
  const onCellValueChanged = useCallback((event) => {
    ...
    fetchx(API_URL2 + `/updatetranGroups?id=${event.data.id}`, {
      method: "PUT",
      body: updatedItem,
      headers: {
  ...
  function Confirm(event) {
    ...
    // console.log(updatedItem)
    fetchx(API_URL2 + `/updatetranGroups`, {
      method: "PUT",
      body: updatedItem,
      headers: {
        "Content-type": "application/json",
  ...
  useEffect(() => {   
    ...
    fetchx(API_URL2 + "/getReservationPreferenceGroup?hotelID=1")
      .then((result) => result.json())
  ...
  const onSubmit = (data) => {
    setData(data);
      ...
      let res = fetchx(API_URL2 + "/addReservationPreferencesGroups", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
...
<div>
        <Modal  isOpen={marketGroup} toggle={() => setMarketGroup(!marketGroup)}className="modal-lg"  >
          <ModalHeader className="modal-lg"toggle={() => setMarketGroup(!marketGroup)} >
           Edit PreferenceGroup Details
          </ModalHeader>
          <ModalBody className="pb-3 px-sm-1 mx-20">            
            <EditpreferenceGroup data1={filldata} />
          </ModalBody>
        </Modal>
      </div>
      {hotelLive === 0 && (<Card>
      <CardHeader><h4><b> Add PreferenceGroup</b></h4></CardHeader>
     </Card>)}
      {hotelLive === 0 && ( <p style={{color: "grey"}}> 
      <b><h4> Instructions:  </h4> </b>
      1.Please refer to the sample file before uploading the data. You can access the sample file by clicking the  <b>"PreferenceGroup File" </b> button. Please ensure that data insertion follows the established pattern.<br/> Kindly refrain from altering the file structure.<br/>
