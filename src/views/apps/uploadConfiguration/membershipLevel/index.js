...
// ** Reactstrap Imports
import { Card, Form, Label, Button, CardBody, CardTitle, CardHeader, Row, Col, InputGroup,  Input,Modal,ModalHeader,ModalBody  } from 'reactstrap'
...
import EditMarketGroup from "./editMembershiplevel"
...
const MembershipLevel = () => {
  ...
  const [columnDefs, setColumnDefs] = useState([
    ...
    {headerName: 'Membership Level',field: 'membershipLevel',suppressSizeToFit: true,cellStyle: { "textAlign": "center", "backgroundColor": "#F1E39B" },
  },
    {headerName: 'Description',field: 'description',      cellStyle: { "textAlign": "center", "backgroundColor": "pink" },
  },
  ...
  //   setBasePriceID(ID)
    ...
    setPopUp("Do You  Want to Change Membership Level Status ?");
  ...
  const updatedItem = JSON.stringify({            
    ...
    //  console.log(updatedItem)
     fetchx(API_URL2+`/updateMembershipLevels?id=${event.data.id}`, {
     method: 'PUT',
     body: updatedItem,
     headers: {
...
// console.log(updatedItem)
fetchx(API_URL2+ `/updateMembershipLevels`, {
method: 'PUT',
body: updatedItem,
headers: {
'Content-type': 'application/json'
...
useEffect(() => {
  ...
  const onSubmit = data => {
    ...
    if (data.membershipLevel!== null && data.description !== null ) {
        ...
        let res = fetchx(API_URL2 +"/addmembershiplevel", {
          method: "POST",
          headers: { 'Content-Type': 'application/json' },
          body: createasset
        }).then(data => data.json())
  ...
  return (
   ...
   <div>
      ...
      {hotelLive === 0 && (<Card>
      <CardHeader><h4><b> Add Membership Level</b></h4></CardHeader>
     </Card>)}
      {hotelLive === 0 && ( <p style={{color: "grey"}}> 
      <b><h4> Instructions:  </h4> </b>
      1.Please refer to the sample file before uploading the data. You can access the sample file by clicking the  <b>"Membership Level File" </b> button. Please ensure that data insertion follows the established pattern.<br/> Kindly refrain from altering the file structure.<br/>
