...
import EditMarketGroup from "./editreservationType"
...
const ReservationTypeData = () => {
  ...
  useEffect(() => {
    ...
    fetchx(API_URL2 +'/getReservationTypeData?hotelID=10')
  ...
  const onSubmit = data => {
    setData(data)
      ...
      let res = fetchx(API_URL2 + "/addReservationTypeData", {
        method: "POST",
        headers: { 'Content-Type': 'application/json' },
      ...
      .then((res) => {
        ...
        if(res['statusCode']==200){
          fetchx(API_URL2 + '/getReservationTypeData?hotelID=1')
          .then(result => result.json())
          .then(rowData => {setRowData(rowData['data'])
  ...
  return (
   ...
   <div>
      ...
      {hotelLive === 0 && (<Card>
      <CardHeader><h4><b> Add Reservation Type</b></h4></CardHeader>
     </Card>)}
      {hotelLive === 0 && ( <p style={{color: "grey"}}> 
      <b><h4> Instructions:  </h4> </b>
      1.Please refer to the sample file before uploading the data. You can access the sample file by clicking the  <b>"Reservation Type File" </b> button. Please ensure that data insertion follows the established pattern.<br/> Kindly refrain from altering the file structure.<br/>
