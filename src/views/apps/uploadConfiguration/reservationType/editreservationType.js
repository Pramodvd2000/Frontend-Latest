  ...
  const onSubmit = (data) => {
    setData(data);
      let createmarketGroup = JSON.stringify({
        "id": data1.id,
        "reservationTypeCode": data.reservationTypeCode1,
        "description": data.description1,
        
          }); 
      let columnsToUpdate = createmarketGroup
      let res = fetchx(API_URL2 + `/updateReservationType?id=${data1.id}`,
        {
          method: "PUT",
        ...
        .then((resp) => {
            ...
            const swalInstance = MySwal.fire({
             text: 'Reservation Type Code Details Updated Successfully!',
             icon: 'success',
         ...
         swalInstance.then((result) => {
           if (result.isConfirmed) {      
            ...
            const swalInstance = MySwal.fire({
                text: 'Reservation Type Code Details Not Updated!',
                icon: 'error',
                buttonsStyling: false,
                confirmButtonText: 'Close',
                customClass: {
  ...
  return (
    <div>
        <Form onSubmit={handleSubmit(onSubmit)}>
            ...
            <Col md="3" sm="12">
                <div className="mb-1">
                  <Label className="form-label" for="reservationTypeCode1">
                  Reservation Type Code  <spam style={{ color: "red" }}>*</spam>
                  </Label>
                  <Controller
                    defaultValue={data1["reservationTypeCode"]}
                    control={control}
                    id="reservationTypeCode1"
                    name="reservationTypeCode1"
                    render={({ field }) => (
                      <Input
                        required
                        placeholder="Name"
                        invalid={errors.reservationTypeCode1 && true}
                        {...field}
                      />
                    )}
                  />
