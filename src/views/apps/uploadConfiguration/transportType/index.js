...
import { Card, CardHeader,Form, Row, Col, Label, Button, CardBody,Input, InputGroup ,Modal,ModalHeader,ModalBody } from 'reactstrap'
...
import EditMarketGroup from "./editTransportType"
...
const Origin = () => {  
  ...
  useEffect(() => {
    ...
    fetchx(API_URL2 +'/getTransportType?hotelID=10')
  ...
  // useEffect(() => {
  //   fetchx(API_URL2 + '/getTransportTypeOptions?hotelID=10')
  //     .then(result => result.json())
  //     .then(rowData => {
  //       const labels = rowData['data'].map(item => item.label);
  //       setRowData(labels);
  //     });
  // }, []);

  console.log(rowData)
//  const transportLabels = jsonData.data.map(item => item.label);
...
const EditData = (rowData)=>{
  ...
  setMarketGroup(!marketGroup)
   ...
   const [columnDefs, setColumnDefs] = useState([
    ...
    {headerName: 'Transport ',field: 'transportType',suppressSizeToFit: true,cellStyle: {'textAlign': 'center','backgroundColor': '#F1E39B'}, maxWidth: 200 },
    {
      headerName: "Action",
      field: "numAvlRooms",
  ...
  const onSubmit = data => {
    setData(data)
      let createmarketGroup = JSON.stringify({
        ...
        "transportType": data.block,
      })
      // console.log(createmarketGroup)
      let res = fetchx(API_URL2 + "/addTransportType", {
        method: "POST",
        headers: { 'Content-Type': 'application/json' },
      ...
      .then((res) => {
        if(res['statusCode']==200){
          ...
          const swalInstance = MySwal.fire({
            text: 'Transport Type Added Successfully!',
            icon: 'success',
...
<div>
      ...
      {hotelLive === 0 && (<Card>
      <CardHeader><h4><b> Add Transport Type</b></h4></CardHeader>
     </Card>)}
      {hotelLive === 0 && ( <p style={{color: "grey"}}> 
      <b><h4> Instructions:  </h4> </b>
      1.Please refer to the sample file before uploading the data. You can access the sample file by clicking the  <b>"Transport Type File" </b> button. Please ensure that data insertion follows the established pattern.<br/> Kindly refrain from altering the file structure.<br/>
    ...
    {hotelLive === 0 && (
     ...
     { hotelLive === 1 && ( <UncontrolledAccordion>
      <AccordionItem>
        <AccordionHeader targetId='1'><h4><b> Add Transport Type</b></h4></AccordionHeader>
