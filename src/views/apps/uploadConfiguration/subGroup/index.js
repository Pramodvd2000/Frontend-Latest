...
// ** Reactstrap Imports
import { Card, Form, Label, Button, CardBody, CardTitle, CardHeader, InputGroup,Col, Row,Input } from 'reactstrap'
...
const SubGroup = () => {
  ...
  const [columnDefs, setColumnDefs] = useState([
    ...
    { headerName: 'Group Name', field: 'groupCode',suppressSizeToFit: true,cellStyle: {'text-align': 'center','background-color': '#F1E39B'}, maxWidth: 140 },
...
const EditData = (rowData)=>{
  ...
  //   setBasePriceID(ID)
    ...
    setPopUp("Do You  Want to Change SubGroup Status ?");
  ...
  const updatedItem = JSON.stringify({            
    ...
    //  console.log(updatedItem)
     fetchx(API_URL2+`/updateSubGroups?id=${event.data.id}`, {
     method: 'PUT',
     body: updatedItem,
     headers: {
...
// console.log(updatedItem)
fetchx(API_URL2+ `/updateSubGroups`, {
method: 'PUT',
body: updatedItem,
headers: {
'Content-type': 'application/json'
...
setPopUp(false)
fetchx(API_URL2 + '/getSubGroup?hotelID=1')
.then(result => result.json())
.then(rowData =>{
...
useEffect(() => {
  ...
  const onSubmit = data => {
    ...
    if (data.subGroup!== null && data.description !== null && data.groupID !== null ) {
        ...
        let res = fetchx(API_URL2 +"/addSubGroup", {
          method: "POST",
          headers: { 'Content-Type': 'application/json' },
          body: subGroup
        }).then(data => data.json())
        .then((res) => {
          ...
          if(res['statusCode']==200){
            ...
            // console.log(rowData['data'])
            const swalInstance = MySwal.fire({
              text: 'Sub Group  Added Successfully!',
              icon: 'success',
              buttonsStyling: false,
              confirmButtonText: 'Close',
              allowOutsideClick: false, 
...
<div>
        <Modal  isOpen={marketGroup} toggle={() => setMarketGroup(!marketGroup)}className="modal-lg"  >
          ...
          <ModalBody className="pb-3 px-sm-1 mx-20">            
            <EditSubGroup data1={filldata} />
          </ModalBody>
        </Modal>
      </div>
      
      {hotelLive === 0 && (<Card>
      <CardHeader><h4><b> Add SubGroup</b></h4></CardHeader>
     </Card>)}
      {hotelLive === 0 && ( <p style={{color: "grey"}}> 
      <b><h4> Instructions:  </h4> </b>
      1.Please refer to the sample file before uploading the data. You can access the sample file by clicking the  <b>"SubGroup File" </b> button. Please ensure that data insertion follows the established pattern.<br/> Kindly refrain from altering the file structure.<br/>
      ...
      <CardBody>
       ...
       <Col md='4' sm='12'>
        ...
        <div className="mb-1">
            ...
            Group Name <spam style={{color:'red'}}>*</spam>
            </Label>
                <Controller
                  id='groupID'
                  control={control}
