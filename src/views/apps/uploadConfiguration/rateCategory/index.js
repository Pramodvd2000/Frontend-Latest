 ...
 const [open, setOpen] = useState('')
  ...
  const [columnDefs, setColumnDefs] = useState([
    ...
    {headerName: 'Description',field: 'description',cellStyle: {'text-align': 'center','background-color': 'pink'}, maxWidth: 140},         
       {headerName: 'Rate Class ',field: 'rateClass',cellStyle: {'text-align': 'center','background-color': '#F1E39B'}, maxWidth: 140},
  ...
  //   setBasePriceID(ID)
    ...
    setPopUp("Do You  Want to Change Rate Category Status ?");
  ...
  const updatedItem = JSON.stringify({            
    ...
    //  console.log(updatedItem)
     fetchx(API_URL2+ `/updateRateCategory?id=${event.data.id}`, {
     method: 'PUT',
     body: updatedItem,
     headers: {
...
// console.log(updatedItem)
fetchx(API_URL2+ `/updateRateCategory`, {
method: 'PUT',
body: updatedItem,
headers: {
'Content-type': 'application/json'
...
setPopUp(false)
fetchx(API_URL2 + '/getRateCategory?hotelID=1')
.then(result => result.json())
.then(rowData =>{
...
useEffect(() => {
  ...
  const onSubmit = data => {
    ...
    if (data.rateCategory.length && data.description.length) {
        ...
        let res = fetchx(API_URL2 + "/addRateCategory", {
          method: "POST",
          headers: { 'Content-Type': 'application/json' },
          body: createasset
        }).then(data => data.json())
        .then((res) => {
          ...
          if(res['statuscode']==200){
            ...
            // console.log(rowData['data'])
            const swalInstance = MySwal.fire({
              text: 'Rate Category  Added Successfully!',
              icon: 'success',
              buttonsStyling: false,
              confirmButtonText: 'Close',
              customClass: {
      ...
      <div>
        <Modal  isOpen={marketGroup} toggle={() => setMarketGroup(!marketGroup)}className="modal-lg"  >
          <ModalHeader className="modal-lg"toggle={() => setMarketGroup(!marketGroup)} >
           Edit Rate Category Details
          </ModalHeader>
          <ModalBody className="pb-3 px-sm-1 mx-20">            
            <EditRateCategory data1={filldata} />
          </ModalBody>
        </Modal>
      </div>

       {hotelLive === 0 && (<Card>
      <CardHeader><h4><b> Add Rate Category</b></h4></CardHeader>
     </Card>)}
      {hotelLive === 0 && ( <p style={{color: "grey"}}> 
      <b><h4> Instructions:  </h4> </b>
      1.Please refer to the sample file before uploading the data. You can access the sample file by clicking the  <b>"Rate Category File" </b> button. Please ensure that data insertion follows the established pattern.<br/> Kindly refrain from altering the file structure.<br/>
