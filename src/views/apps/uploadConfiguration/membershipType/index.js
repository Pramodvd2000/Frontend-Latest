...
import EditMarketGroup from "./editMembershipType"
...
const membershipType = () => {
  ...
  const updatedItem = JSON.stringify({            
    ...
    //  fetchx(API_URL2+`/updateMembershipTypes?id=${event.data.id}`, {
    //  method: 'PUT',
    //  body: updatedItem,
    //  headers: {
...
// console.log(updatedItem)
fetchx(API_URL2+ `/updateMembershipTypes`, {
method: 'PUT',
body: updatedItem,
headers: {
'Content-type': 'application/json'
...
setPopUp(false)
fetchx(API_URL2 + '/getMembershipType?hotelID=1')
...
useEffect(() => {
  ...
  const onSubmit = (data) => {
    ...
    if (data.membershipType.length && data.description.length) {
     ...
     console.log(createmembershipType);
      let res = fetchx(API_URL2 +"/addmembershipType", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  return (
   ...
   <div>
      ...
      {hotelLive === 0 && (<Card>
      <CardHeader><h4><b> Add Membership Type</b></h4></CardHeader>
     </Card>)}
      {hotelLive === 0 && ( <p style={{color: "grey"}}> 
      <b><h4> Instructions:  </h4> </b>
      1.Please refer to the sample file before uploading the data. You can access the sample file by clicking the  <b>"Membership Type File" </b> button. Please ensure that data insertion follows the established pattern.<br/> Kindly refrain from altering the file structure.<br/>
