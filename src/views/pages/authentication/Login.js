...
const Login = () => {
  ...
  const onSubmit = data => {
    // let loginURL = API_URL;   //Comment this code for live
    ...
    // if(data.userrole.value == '2'){
    //   loginURL = DASHBOARD_URL
    // }
    // console.log(loginURL)
    // For live change loginURL to API_URL
    ...
    if(resp['userlevel'] === undefined){
      ...
      console.log(resp.userrole)
        ...
        // if (Object.values(data).every(field => field.length > 0)) {
          useJwt
            .login({ email: "a@gmail.com", password: "vidya12", extraKey: resp.userrole })
            .then(res => {
              const data = { ...res.data.userData, accessToken: res.data.accessToken, refreshToken: res.data.refreshToken }
              dispatch(handleLogin(data))
              ability.update(res.data.userData.ability)
              ...
              {/* Comment this for live */}
               {/* <div className='mb-1'>
                <Label className='form-label' for='userrole' >
                  Role <spam style={{ color: 'red' }}>*</spam>
                </Label>
                <Controller
                  id='userrole'
                  control={control}
