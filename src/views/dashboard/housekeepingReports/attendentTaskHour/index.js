...
function App() {
  ...
  const [columnDefs] = useState([
    ...
    {
      headerName: 'Total No. Hr',
      field: 'NoTotalHr',
      suppressSizeToFit: true,
      Width: 200,
    },
    {
      headerName: 'Total Active Hr',
      field: 'totalActiveHr',
      suppressSizeToFit: true,
      Width: 200,
    },
  ...
  useEffect(() => {
    fetch(Houskeeping_URL + `/getAttendentTaskHours?hotelID=${id}`)
      .then(result => result.json())
      .then(rowData => {
  ...
  const sevenDays = useCallback(() => {
    fetch(Houskeeping_URL + `/getAttendentTaskHoursLastSevenDays?hotelID=${id}`)
      .then(result => result.json())
      .then(rowData => {
        setRowData(rowData['data'])
      })
  ...
  const onSubmit = data => {
    ...
    fetch(Houskeeping_URL + `/getAttendentTaskHours?fromDate=${fromDate}&toDate=${toDate}`, {
      method: "GET",
