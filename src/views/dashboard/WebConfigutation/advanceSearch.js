...
function AdvanceSearch(props) {
  ...
  const [noFilterApplied, setNoFilterApplied] = useState(true);
  ...
  const timeRangeOptions = [
    ...
    { value: 'entireDatabase', label: 'Entire Database' },
  ];
  ...
  const handleTimeRangeChange = (selectedOption) => {
    const value = selectedOption ? selectedOption.value : ''; // Safely handle undefined selectedOption
    setTimeRangeFilter(value);
    setClickCount(true);

    if (value !== '') {
      setArrivalDateFilter(null);
      // setDepartureDateFilter(null); // Uncomment if needed
    } else {
      setCustomYear('');
    }
  };
  ...
  const handleFilterTypeChange = (filterTypeOptions) => {
    setFilterType(filterTypeOptions ? filterTypeOptions.value : ''); // Set filter type based on selected option, or clear it if none
    setClickCount(true);
    setFilterValue(''); // Clear filter value when switching types
  };


  // Handles filter type selection (dropdown)
  ...
  const resetFilters = () => {
    ...
    reset({ filterType: null });
  };



  const applyFilters = async () => {
    // Validation checks for filter values
    if (timeRangeFilter === 'custom' && !arrivalDateFilter) {
      handleError("Please select custom arrival date.");
      return;
    }

    if (!filterValue && !arrivalDateFilter && !timeRangeFilter && !customYear) {
      handleError("Please select and enter atleast one filter.");
      return;
    }

    if (filterType !== 'bookingID' && !timeRangeFilter) {
      handleError("Arrival date filter is required when a filter type is selected.");
      return;
    }

    // Validate filter values based on filterType
    if (filterType === 'roomNumber') {
      // Remove non-digit characters
      const numericValue = filterValue.replace(/\D/g, '');

      if (numericValue.length < 2) {
        handleError("Enter at least 2 digits.");
        return;
      } else if (numericValue.length > 8) {
        handleError("More than 8 digits not allowed.");
        return;
      }

      setFilterValue(numericValue); // Update the filter value after validation
    }
    //  else if (filterType === 'guestName' || filterType === 'companyName') {
    //   if (filterValue.length < 3) {
    //     handleError("Enter at least 3 characters.");
    //     return;
    //   }
    // }

    // Around line 280, update the validation:
    else if (filterType === 'guestName' || filterType === 'companyName' || filterType === 'email') {
      if (filterValue.length < 3) {
        handleError("Enter at least 3 characters.");
        return;
      }
    ...
    try {
      ...
      if (filterType === 'roomNumber' && filterValue) {
        queryParams.append('roomNumber', filterValue);
      }
      if (filterType === 'bookingID' && filterValue) {
        queryParams.append('bookingID', filterValue);
      }
      if (filterType === 'guestName' && filterValue) {
        queryParams.append('guestName', filterValue);
      }
      if (filterType === 'companyName' && filterValue) {
        queryParams.append('accountName', filterValue);
      }

      // Around line 315-320, add these conditions:
      if (filterType === 'email' && filterValue) {
        queryParams.append('email', filterValue);
      }
      console.log('Selected Filter Type:', filterType);
      if (filterType === 'MobileNo' && filterValue) {
        queryParams.append('MobileNo', filterValue);
      }
      if (arrivalDateFilter) {
        const fromdate = Moment(String(new Date(arrivalDateFilter))).format('YYYY-MM-DD');
        queryParams.append('arrivalDate', fromdate);

        // Calculate future date 3 months ahead and format both dates
        arrivalDateFormatted = Moment(arrivalDateFilter).format('DD/MM/YYYY');
        futureDateFormatted = Moment(arrivalDateFilter).add(3, 'months').format('DD/MM/YYYY');
      }

      if (timeRangeFilter) {
        queryParams.append('timeRange', timeRangeFilter);
        if (timeRangeFilter === 'custom' && customYear) {
          queryParams.append('customYear', customYear);
        }
      }

      if (queryParams.toString() === '') {
        handleError("Please provide at least one valid filter.");
        return;
      }

      const response = await fetchx(`${API_URL}/getReservationForAdvancedSearch?${queryParams.toString()}`);
    ...
    // Store the formatted dates to be displayed later
    setDisplayDateRange(arrivalDateFormatted && futureDateFormatted ?
      `This data is for arrivals between <strong>${arrivalDateFormatted}</strong> and <strong>${futureDateFormatted}</strong>` : '');
  };
  ...
  const onGridReady = (params) => {
    if (loading) {
      ...
      setTimeout(() => params.api.setOverlayNoRowsTemplate('<span>No matched data found</span>'), 0);
    } else if (noData) {
      params.api.showNoRowsOverlay();
      setTimeout(() => params.api.setOverlayNoRowsTemplate('<span>No matched data found</span>'), 0);
    } else {
...
`;
  ...
  const handleInputChange2 = (e) => {
    ...
    if (gridRef.current) {
      const filterModel = gridRef.current.getFilterModel();
      filterModel['departureDateFilter'] = {
        filter: e.target.value,
        type: 'date',
      };
      gridRef.current.setFilterModel(filterModel);
  ...
  className={classnames('react-select', {
    'is-invalid': data !== null && data.filterType === null // Validation check
  })}
  onChange={(selectedOption) => {
    onChange(selectedOption ? selectedOption.value : ''); // Update react-hook-form value
    setFilterType(selectedOption ? selectedOption.value : ''); // Update local state
    setClickCount(true);
    setFilterValue(''); // Clear filter value when filter type changes
    // reset({ timeRangeFilter: null });
     setTimeRangeFilter();
  }}
  value={filterTypeOptions.find(option => option.value === value) || null} // Ensure clearing works
  ref={ref}
/> */}

        <Col md='2' sm='12'>
          <div className='mb-1'>
            <Label className='form-label' for='filterType'>
              Select Filter Type <span style={{ color: 'red' }}>*</span>
            </Label>
            <Controller
              id='filterType'
              required
              ...
              render={({ field: { onChange, value, ref } }) => (
                ...
                <Select
                  ...
                  onChange={(selectedOption) => {
                    onChange(selectedOption ? selectedOption.value : null); // Update react-hook-form value
                    setFilterType(selectedOption ? selectedOption.value : null); // Update local state
                    setClickCount(true);
                    setFilterValue(''); // Clear filter value when filter type changes
                    setValue('timeRangeFilter', null); // Reset timeRangeFilter using react-hook-form
                    setTimeRangeFilter(null); // Update local state
                  }}
                  value={filterTypeOptions.find(option => option.value === value) || null}
                  ref={ref}
                />
        ...
        <Col md="2" sm="12" className="mb-1">
          {filterType && (
            <div>
              <Label className="form-label" for="filterValue">
                {filterType === 'roomNumber' ? 'Enter Room Number' :
                  filterType === 'bookingID' ? 'Enter Booking ID' :
                    filterType === 'guestName' ? 'Enter Guest Name' :
                      filterType === 'companyName' ? 'Enter Company Name' :
                        'Enter Value'}
              </Label>
              <Input
                type={filterType === 'roomNumber' ? 'number' : 'text'}
                id="filterValue"
                value={filterValue}
                onChange={handleFilterInputChange}
                placeholder={
                  filterType === 'roomNumber' ? 'Room Number' :
                    filterType === 'bookingID' ? 'Booking ID (e.g., 1234-5)' :
                      filterType === 'guestName' ? 'Guest Name' :
                        filterType === 'companyName' ? 'Company Name' :
                          'Enter value'
                }
                min={filterType === 'roomNumber' ? '10' : undefined} // 2 digits minimum
                maxLength={filterType === 'roomNumber' || filterType === 'bookingID' ? '8' : undefined} // 8 digits max
              />
            </div>
          )}
        </Col>
        ...
        {/* 
{ (filterType === 'roomNumber'  || filterType === 'guestName' || filterType === 'companyName') && (
  <Col md="2" sm="12" className="mb-1">
    <Label className="form-label" for="timeRangeFilter">
      Arrival Date Filter <span style={{ color: 'red' }}>*</span>
    </Label>
    <Controller
      id="timeRangeFilter"
      control={control}
      ...
      render={({ field: { onChange, onBlur, value, ref } }) => (
        <Select
          ...
          theme={selectThemeColors}
          className={classnames('react-select', {
            'is-invalid': data !== null && data.timeRangeFilter === null,
          })}
          onChange={(selectedOption) => {
            onChange(selectedOption); 
            handleTimeRangeChange(selectedOption); 
          }}
          value={timeRangeOptions.find(option => option.value === value)} 
          ref={ref} 
        />
      )}
    />
    ...
    <Controller
      ...
      render={({ field: { onChange, value, ref } }) => (
        <Select
          ...
          onChange={(selectedOption) => {
            onChange(selectedOption); // Update react-hook-form
            handleTimeRangeChange(selectedOption); // Additional handling if needed
          }}
          value={timeRangeOptions.find(option => option.value === value) || null} // Handle both form value and null reset
          ref={ref}
        />
      )}
    />
        ...
        {/* {(filterType === 'roomNumber' || filterType === 'guestName' || filterType === 'companyName') && ( */}
        {(filterType === 'roomNumber' || filterType === 'guestName' || filterType === 'companyName' || filterType === 'email' || filterType === 'MobileNo') && (
          ...
          <Col md="2" sm="12" className="mb-1">
            ...
            <Controller
              ...
              render={({ field: { onChange, value, ref } }) => (
                <Select
                  isClearable
                  options={timeRangeOptions} // Available time range options
                  classNamePrefix="select"
                  theme={selectThemeColors}
                  className={classnames('react-select', {
                    'is-invalid': data !== null && data.timeRangeFilter === null,
                  })}
                  placeholder="Select..." // Placeholder for the dropdown
                  onChange={(selectedOption) => {
                    console.log('Selected Option:', selectedOption); // Debugging log to check selected option
                    onChange(selectedOption); // Update react-hook-form value
                    handleTimeRangeChange(selectedOption); // Additional handler if needed
                  }}
                  value={timeRangeOptions.find(option => option.value === value?.value) || null} // Ensure the correct option is selected
                  ref={ref} // Ref handling for react-hook-form
                />
              )}
            />
          </Col>
        ...
        {timeRangeFilter === 'custom' && filterType !== 'bookingID' && (
          <Col md="2" sm="12" className="mb-1">
            <Label className="form-label" for="arrivalDateFilter">Arrival Date:</Label>
            <Flatpickr
              id="arrivalDateFilter"
              value={arrivalDateFilter}
              options={{ dateFormat: 'Y-m-d' }}
              onChange={([date]) => {
                setArrivalDateFilter(date);
                setClickCount(true);
              }}
              placeholder="Select Arrival Date"
              className="form-control"
              disabled={(!!timeRangeFilter && timeRangeFilter !== 'custom') || !filterType || filterType === 'bookingID'} // Disable if time range is selected
      ...
      <Row className="mb-2 align-items-center">
        <Col md="auto" sm="14" className="mb-1">
          <Button color="primary" onClick={applyFilters} disabled={!clickCount}>
            Apply Filters
          </Button>
        </Col>
        <Col md="auto" sm="14" className="mb-1">
        ...
        <Col className="mb-1 text-right">
          {arrivalDateFilter && displayDateRange && (
            <p style={{ fontSize: '14px', color: 'black', textAlign: 'right' }}
              dangerouslySetInnerHTML={{ __html: displayDateRange }}>
            </p>
          )}
        </Col>
      </Row>
