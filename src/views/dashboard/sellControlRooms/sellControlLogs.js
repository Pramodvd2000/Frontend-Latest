...
const InventoryLogs = (data) => {
    ...
    useEffect(() => {
        ...
        fetchx(API_URL + '/getSellControlRoomsLogs',)
            .then((res) => res.json())
            .then(postres => {
    ...
    useEffect(() => {
        ...
        if (fromDate !== undefined && toDate !== undefined) {
            if (Array.isArray(fromDate) && Array.isArray(toDate) &&
                ...
                fetch(API_URL + "/getSellControlRoomsLogs?fromDate=" + fromDateFormat + "&toDate=" + toDateFormat)
                ...
                fetch(API_URL + "/getSellControlRoomsLogs")
                    .then((result) => result.json())
                    .then((rowData) => {
                        setLogData(rowData["data"]);
                        // console.log(rowData)
    ...
    const [columnDefs1, setColumnDefs1] = useState([
        ...
        {
            headerName: 'From Date', field: 'fromDate', width: 120, cellRenderer: (params) => {
                if (params.data && params.data.fromDate) {
                    const formattedDate = format(new Date(params.data.fromDate), 'dd MMM yy');
        ...
        {
            headerName: 'To Date', field: 'toDate', width: 120, cellRenderer: (params) => {
                if (params.data && params.data.toDate) {
                    const formattedDate = format(new Date(params.data.toDate), 'dd MMM yy');
                    return formattedDate;
                } else {
                    return "";
                }
        ...
        { headerName: 'No. Sell Ctrl Rooms', field: 'numSellCtrlRooms', width: 160 },
        { headerName: 'Room Type', field: 'roomType', width: 130 },
        // { headerName: 'Online Status', field: 'onlineStatus', width: 130 },
        {
            ...
            cellStyle: params => {
                if (params.value === 'Offline') {
                    return { backgroundColor: '#f8d7da', color: '#721c24', fontWeight: 'bold' }; // light red
                } else if (params.value === 'Online') {
                    return { backgroundColor: '#d4edda', color: '#155724', fontWeight: 'bold' }; // light green
                }
                return null;
            }
        },
        { headerName: 'Created By', field: 'createdUser', width: 140 },
        { headerName: 'Created At', field: 'createdAt', width: 110 },
