...
const Block = () => {
  ...
  const [columnDefs, setColumnDefs] = useState([
    ...
    {
      headerName: 'Birthday Date', field: 'dob', suppressSizeToFit: true, width: 160,
      cellRenderer: (params) => {
        if (params.data && params.data.dob) {
          const formattedDate = Moment(params.data.dob).format("DD-MM-YYYY");
          return formattedDate;
        } else {
          return "";
        }
  ...
  const onBtnExport = () => {
    const params = {
      fileName: 'IHG Birthday Report.xlsx',
      sheetName: 'Sheet1',
    };
  ...
  return (
    <div>
      <Card>
        <CardHeader>
          <CardTitle tag='h4'>In House Guest Birthdays Report</CardTitle>
        </CardHeader>
        <CardBody>
