...
const longStayGuestReport = () => {
  ...
  const [columnDefs, setColumnDefs] = useState([
    ...
    { headerName: 'Booking ID', field: 'BookingID', suppressSizeToFit: true, maxWidth: 170 },
    ...
    { headerName: 'Reservation Status', field: 'reservationStatus', suppressSizeToFit: true, maxWidth: 160,  autoHeaderHeight: true ,wrapHeaderText: true },

    { headerName: 'Company', field: 'companyName', suppressSizeToFit: true, maxWidth: 179 },
    {
      headerName: "Arrival",
      field: "arrivalDate",
  ...
  const onButtonExport = () => {
    const params = {
      fileName: 'Long Staying Guest Report.xlsx',
      sheetName: 'Sheet1',
    };
  ...
  const onSubmit = data => {
    ...
    if (flag1 == true) {
      let res = fetchx(DASHBOARD_URL + "/getLongStayGuestReport", {
        method: "POST",
  ...
  const downloadPDF = async (data) => {
    ...
    try {
      ...
      const textToCenter = "Long Staying Guest Report";
      ...
      saveAs(pdfBlob, 'Longstay guest Report.pdf');
    } catch (error) {
  ...
  console.log(document.getElementById('todate'), document.getElementById('frmdate '))

  const generateExcel = () => {
    if (filterFromDate && filterToDate) {
      const workbook = new ExcelJS.Workbook();
      const worksheet = workbook.addWorksheet('Long Staying Guest Report');
      ...
      worksheet.addRow(['Report Name:', 'Long Staying Guest Report']);
      ...
      workbook.xlsx.writeBuffer().then((buffer) => {
        ...
        saveAs(blob, `Long Staying Guest Report_${formattedDate}.xlsx`);
      }).catch((error) => {
  ...
  return (
    <div>
      ...
      <Card>
        <CardHeader>
          <CardTitle tag='h4'>Long Staying Guest Report</CardTitle>
        </CardHeader>
        <CardBody>
...
export default longStayGuestReport
