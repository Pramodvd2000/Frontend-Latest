...
const FOSettlementbillwiseReport = () => {
  ...
  const [columnDefs, setColumnDefs] = useState([
    { headerName: 'User', field: 'first_name', suppressSizeToFit: true, maxWidth: 170,  },
    ...
    cellRenderer: (params) => {
      ...
      if (params.data && params.data.DateTime) {
        ...
        const formattedDate = Moment(params.data.DateTime).format("DD.MM.YYYY hh:mm:ss");
    ...
    { headerName: 'Bill No', field: 'billNoDisplay', suppressSizeToFit: true, maxWidth: 180},
    ...
    { headerName: 'Payment Type', field: 'description', suppressSizeToFit: true, maxWidth: 170 },
    {
      headerName: 'Amount',
      field: 'base_amount',
  ...
  const onButtonExport = () => {
    const params = {
      fileName: 'FO Settlement Billwise Report.xlsx',
      sheetName: 'Sheet1',
    };
  ...
  const onSubmit = data => {
    ...
    if (flag1 == true) {
      let res = fetchx(DASHBOARD_URL + "/getFOSettlementsbillwiseReport", {
        method: "POST",
        headers: { 'Content-Type': 'application/json' },
  ...
  const downloadPDF = async () => {
    ...
    try {
      ...
      const textToCenter = "FO Settlement Billwise Report";
      ...
      for (const billNoDisplay in groupedRows) {
        ...
        const totalAmount = groupedRows[billNoDisplay].reduce((total, row) => total + parseFloat(row['base_amount'] || 0), 0).toFixed(2);

        // const formattedTotalAmount = parseFloat(totalAmount).toLocaleString('en-US', {
      ...
      saveAs(pdfBlob, 'Fo_Settlement_Bill_wise.pdf');
    } catch (error) {
...
const generateExcel = () => {
  if (filterFromDate && filterToDate) {
    ...
    const worksheet = workbook.addWorksheet('FO Settlements Bill Wise Report');
    ...
    worksheet.addRow(['Report Name:', 'FO Settlements Bill Wise Report']);
    ...
    workbook.xlsx.writeBuffer().then((buffer) => {
      ...
      saveAs(blob, `FO Settlements Bill Wise Report_${formattedDate}.xlsx`);
    }).catch((error) => {
  ...
  return (
    <div>
      ...
      <Card>
        <CardHeader>
          <CardTitle tag='h4'>FO Settlements Bill Wise Report</CardTitle>
        </CardHeader>
        <CardBody>
...
export default FOSettlementbillwiseReport