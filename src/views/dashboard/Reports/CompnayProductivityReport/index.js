...
const ProductivityCompanyReport = () => {
  ...
  const CustomHeaderComponentRate5 = ({ value }) => {
    return (
      <div>
        ARR
  ...
  const [columnDefs, setColumnDefs] = useState([
    ...
    { headerName: 'Date', field: 'date', sort: 'asc', suppressSizeToFit: true, width: 150 },

    { headerName: 'Company', field: 'accountName', suppressSizeToFit: true, width: 250, rowGroup: true },

    { headerName: 'Nights', field: 'No_of_Nights', suppressSizeToFit: true, maxWidth: 140, sort: 'asc', aggFunc: 'sum', headerComponentFramework: CustomHeaderComponentRate1, valueFormatter: formatNumber, },
    { headerName: 'Room Revenue', field: 'room_revenue', suppressSizeToFit: true, maxWidth: 150, aggFunc: 'sum', headerComponentFramework: CustomHeaderComponentRate6, valueFormatter: formatNumber, },
    { headerName: 'Package Revenue', field: 'package_revenue', suppressSizeToFit: true, maxWidth: 150, aggFunc: 'sum', headerComponentFramework: CustomHeaderComponentRate7, valueFormatter: formatNumber, },
   ...
   return params.data.ADR;
    ...
    { headerName: 'Tax(for future)', field: 'tax_for_future', suppressSizeToFit: true, maxWidth: 150, aggFunc: 'sum', headerComponentFramework: CustomHeaderComponentRate4, valueFormatter: formatNumber, },

    { headerName: 'Revenue', field: 'revenue', suppressSizeToFit: true, maxWidth: 150, aggFunc: 'sum', headerComponentFramework: CustomHeaderComponentRate2, valueFormatter: formatNumber, },
  ...
  const onButtonExport = () => {
    const params = {
      fileName: 'Profile Company Productivity Report.xlsx',
      sheetName: 'Sheet1',
    };
  ...
  useEffect(() => {
    fetchx(DASHBOARD_URL + "/getGuestProfileCompanyID?hotelID=10")
  ...
  const printGrid = () => {
    ...
    if (gridApi) {
      ...
      const textToCenter = "Profile Productivity Company Report";
      ...
      //   sourceCodeTotalNights[accountName] = (sourceCodeTotalNights[accountName] || 0) + nights;
      // });
        ...
        .forEach((accountName, index) => {
          const rowsForDate = updatedRowData
            ...
            .map((row) => {
              ...
              const formattedArrival = selectedOption === '1'
                ? (row.date && isValidDate(new Date(row.date)) ? formatDateString(new Date(row.date)) : null)
                : (row.date && isValidDate(new Date(row.date)) ? formatDate(new Date(row.date)) : '');


              // Add a helper function to check if a date is valid
          ...
          const totalNights = rowsForDate.reduce((acc, row) => {
            const formattedValue = row[2].replace(/[^0-9.-]+/g, ''); // Remove non-numeric characters, keep the decimal point
          ...
          if (accountName == undefined) {
            ...
            pdf.autoTable({
              head: [columns],
              body: [['', 'Total', formattedTotalNights, formattedTotalRoomRevenue, formattedTotalPackageRevenue, formattedARR, formattedTotalFnBRevenue, formattedTotalTax, formattedTotalRevenue]],
              startY: dateY + 43,
              margin,
              didParseCell: (data) => {
            ...
            pdf.text(`${(accountName)}`, 10, dateY + 37, { width: 500, align: 'left' });
            pdf.autoTable({
              head: [columns],
              body: [...rowsForDate, ['', 'Total', formattedTotalNights, formattedTotalRoomRevenue, formattedTotalPackageRevenue, formattedARR, formattedTotalFnBRevenue, formattedTotalTax, formattedTotalRevenue]],
              startY: dateY + 43,
              margin,
              didParseCell: (data) => {
                if (data.column.index !== 0 && data.column.index !== 1 && data.column.index !== 2) {
      ...
      pdf.text(`Filter From Date: ${fromDate} to To Date: ${toDate}`, 10, dateY + 10, { width: 500, align: 'left' });
      pdf.text(`Company: ${companyNames}`, 10, dateY + 20, { width: 500, align: 'left' });
      ...
      pdf.save("Company Productivity Report.pdf");
    } else {
    }
  };
  ...
  const onSubmit = data => {
    ...
    let createmarketGroup = JSON.stringify({
      ...
      "CompanyID": companyData !== null ? JSON.stringify(companyData) : companyData
    });
    if (flag1 && selectedOption) {
      fetchx(DASHBOARD_URL + "/getCompanyProductivity", {
        method: "POST",
        headers: { 'Content-Type': 'application/json' },
        ...
        .catch(error => {
          setIsButtonClicked(false)
          console.error('Error fetchxxing data:', error);
        });
    }
  ...
  const generateExcel = () => {
    if (filterFromDate && filterToDate) {
      ...
      const worksheet = workbook.addWorksheet('Profile Productivity Company Report'); // Updated worksheet name
      
      // Updated column definitions based on your new headers and fields
      const columns = [
        { header: 'Date', key: 'date', width: 15 },
        { header: 'Company', key: 'accountName', width: 25 },
      ...
      worksheet.addRow(['Report Name:', 'Profile Productivity Company Report']); // Updated report name
      ...
      let selectedFilterMarket = filterMarket && filterMarket.length !== 0
        ? filterMarket.join(', ')
        : 'All';
      worksheet.addRow(['Filter Company:', selectedFilterMarket]);
      worksheet.addRow();
      ...
      workbook.xlsx.writeBuffer().then((buffer) => {
        ...
        saveAs(blob, `Profile Productivity Company Report_${formattedDate}.xlsx`); // Updated file name
      }).catch((error) => {
  ...
  return (
    <div>
      ...
      <Card>
        <CardHeader>
          <CardTitle tag='h4'>Profile Productivity Company Report</CardTitle>
        </CardHeader>
        <CardBody>
...
{/* 
              ...
              >
                ...
                <Button className='me-1' color='primary'  onClick={() => printGrid(data)}>
                  Print PDF
                </Button>
              </div>
            </Row>
...
export default ProductivityCompanyReport
