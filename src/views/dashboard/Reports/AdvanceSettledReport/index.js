...
const advanceSettled  = () => {
  ...
  const [columnDefs, setColumnDefs] = useState([
    ...
    { headerName: 'Base Amount',field: 'base_amount',maxWidth: 120 , aggFunc: 'sum',valueFormatter: formatNumbers,headerComponentFramework: CustomHeaderComponentbaseamount},
  ...
  const onSubmit = data => {
    ...
    if (flag1 == true) {
      let res = fetchx(DASHBOARD_URL + "/getAdvanceSettledReportPMS", {
        method: "POST",
        headers: { 'Content-Type': 'application/json' },
  ...
  const onBtnExport = () => {
    const params = {
      fileName: 'Advance Settled Report.xlsx',
      sheetName: 'Sheet1',
    };
  ...
  const printGrid = () => {
    if (filterFromDate ) {
      ...
      if (gridApi) {
        ...
        pdf.setFontSize(14);
        pdf.setFont('helvetica', 'normal');
        const textToCenter = "Advance Settled Report";
        ...
        pdf.save('Advance Settled Report.pdf');
      } else {
      }
    }
  };
  ...
  const generateExcel = () => {
    if (filterFromDate) {
      ...
      const worksheet = workbook.addWorksheet('Advance Settled Report'); // Changed report name
      ...
      worksheet.addRow(['Report Name:', 'Advance Settled Report']); // Changed report name
      ...
      workbook.xlsx.writeBuffer().then((buffer) => {
        ...
        saveAs(blob, `Advance Settled Report_${formattedDate}.xlsx`); // Changed report name
      }).catch((error) => {
  ...
  return (
    <div>
      <Card>
        <CardHeader>
          <CardTitle tag='h4'>Advance Settled  Report</CardTitle>
        </CardHeader>
        <CardBody>
