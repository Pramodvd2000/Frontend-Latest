...
useEffect(() => {
  ...
  .catch((error) => {
    ...
    fetchx(DASHBOARD_URL + '/getAllAccountManager?hotelID=1')
    .then(result => result.json())
    .then(resp => {
      const responseData = resp['data'];
  
      const accountManagerOptions = responseData.map(item => ({
        value: item.accountManagerID,
        label: `${item.first_name} ${item.last_name}`,
      }));
  
      setAccountManagerOptions(accountManagerOptions);
    })
    .catch(error => {
...
//  ColDef
  ...
  //   const accountManagerID = Array.isArray(data.accountManagerID) ? data.accountManagerID.map(item => item.value) : [];
  //   console.log(data)
  //   setData(data)
  ...
  const ConfirmSubmit = (data) => {
    const accountManagerID = Array.isArray(data.accountManagerID) 
      ? data.accountManagerID.map(item => item.value) 
      : [];
    ...
    if (Array.isArray(data.FromDate) && data.FromDate.length > 0) {
      data.FromDate = Moment(data.FromDate[0]).format('YYYY-MM-DD');
    } else {
      console.error('FromDate is not defined or is empty');
      return;
    }
  
    setLoadingVar(true);
    getAccountManagerProductivityy({ ...data, accountManagerID });
  }
...
const getAccountManagerProductivityy = (data) => {
  ...
  // Directly use the accountManagerID if it's an array
  const accountManagerID = Array.isArray(data.accountManagerID)
    ? data.accountManagerID // No need to map
    : [];

  const accountManagerIDString = accountManagerID.join(','); // Join the IDs into a string

  fetch(DASHBOARD_URL + `/getDashboardAccountManagerProductivityReport?hotelID=1&date=${data.FromDate}&accountManagerID=[${accountManagerIDString}]`)
  .then((result) => result.json())
    .then((response) => {
...
const getAccountManagerWiseSales = (date,accountManagerID,companyIDs) =>{
 companyIDs = Array.isArray(companyIDs)
  ? companyIDs // No need to map
  : [];

// console.log(DASHBOARD_URL + `/getDashboardAccountManagerProductivity?hotelID=1&date=${data.FromDate}&accountManagerID=[${accountManagerIDString}]`);

const companyIDsString = companyIDs.join(',');

  fetch(DASHBOARD_URL + `/getAccountManagerWiseSalesReport?hotelID=1&date=${date}&accountManagerID=${accountManagerID}&companyIDs=[${companyIDsString}]`)
...
const cellClickedListener = useCallback(event => {
  getAccountManagerWiseSales(event.data.day_date,event.data.accountManagerID_id,companyIDs)

  setAccountData(true);
}, [companyIDs]); // Ensure to include all dependencies in the dependencies array
