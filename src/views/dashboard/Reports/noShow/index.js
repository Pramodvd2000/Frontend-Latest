...
const NoShowReport = () => {
  ...
  const [isButtonClicked, setIsButtonClicked] = useState(false);
   ...
   const [columnDefs, setColumnDefs] = useState([
    ...
    {headerName: 'Company/Agent',field: 'accountName',suppressSizeToFit: true, maxWidth: 140 ,wrapText: true,autoHeight:true,},
    ...
    {headerName: 'Departure',field: 'departureDate',suppressSizeToFit: true, maxWidth: 160 ,cellRenderer: (params) => {
  ...
  const onSubmit = data => {
    ...
    setData(data)
      ...
      let res = fetchx(DASHBOARD_URL+"/getNoShowReservationReport", {
        method: "POST",
        headers: { 'Content-Type': 'application/json' },
  ...
  const generateExcel = () => {
    if (filterFromDate && filterToDate) {
      ...
      const worksheet = workbook.addWorksheet('No Show Report');
  
      const columns = [
        { header: 'Booking ID', key: 'bookingID', width: 20 },
        { header: 'Guest Name', key: 'guestName', width: 25 },
        { header: 'Rooms', key: 'numberOfRooms', width: 15 },
        { header: 'Reservation Date', key: 'createdAt', width: 20 },
      ...
      worksheet.addRow(['Report Name:', 'No Show Report']);
      ...
      workbook.xlsx.writeBuffer().then((buffer) => {
        ...
        saveAs(blob, `No Show Report_${formattedDate}.xlsx`);
      }).catch((error) => {
  ...
  return (
    ...
    <Card>
      <CardHeader>
        <CardTitle tag='h4'>No Show Report</CardTitle>
      </CardHeader>
      <CardBody>
