...
const InhouseGuestNationality = () => {
  ...
  const [columnDefs, setColumnDefs] = useState([
    ...
    { headerName: 'Guest Name ', field: 'guestName', suppressSizeToFit: true, width: 260, wrapText: true, autoHeight: true, },
    {
    ...
    { headerName: 'Nationality', field: 'nationalityName', suppressSizeToFit: true, width: 160 },
     {
       headerName: "Arrival",
       field: "arrivalDate",
       suppressSizeToFit: true,
    ...
    { headerName: 'No. Nights', field: 'numOfNights', suppressSizeToFit: true, width: 120 },
    { headerName: 'No. Stays', field: 'numOfStay', suppressSizeToFit: true, width: 120 },
    {headerName: "Room Revenue",field: "roomRevenue",suppressSizeToFit: true,maxWidth: 110,  valueGetter: (params) => params.data?.roomRevenue ?? 0    },
    {headerName: "Package Revenue",field: "packageRevenue",suppressSizeToFit: true,maxWidth: 110,valueGetter: (params) => params.data?.packageRevenue ?? 0},
    {headerName: "Total Revenue",field: "TotalRevenue",suppressSizeToFit: true,maxWidth: 110,valueGetter: (params) => params.data?.TotalRevenue ?? 0},
    {headerName: "Market",field: "marketCode",suppressSizeToFit: true,maxWidth: 110},
  ...
  useEffect(() => {
    ...
    setOpen(true)
    fetchx(DASHBOARD_URL + "/getPastNationality", {
      method: "POST",
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        hotelID: '1',
  ...
  //       // fetch(API_URL + "/getGuestProfileCountry?fromDate=" + fromDateFormat + "&toDate=" + toDateFormat)
  //       //   .then((result) => result.json())
  //       fetchx(DASHBOARD_URL + "/getPastNationality", {
  //         method: "POST",
  //         headers: { 'Content-Type': 'application/json' },
  ...
  const onSubmit = data => {
    ...
    let createmarketGroup = JSON.stringify({
      "hotelID": 1,
      "nationality": companyData.length === 0 ? null : companyData,
    })
    ...
    if (flag1 == true) {
      if (fromDate !== undefined && toDate !== undefined) {
        if (Array.isArray(fromDate) && Array.isArray(toDate) &&
          ...
          let marketGroupObj = JSON.parse(createmarketGroup);
            marketGroupObj.fromDate = fromDateFormat;
            marketGroupObj.toDate = toDateFormat;
            createmarketGroup = JSON.stringify(marketGroupObj);
  ...
  const printGrid = () => {
    ...
    if (gridApi) {
      ...
      pdf.save('Past Guest Nationality Report.pdf');
    }
  };
  ...
  const generateExcel = () => {
    if (filterFromDate) {
      ...
      const worksheet = workbook.addWorksheet('Past Guest Nationality Report');

      const columns = [
        { header: 'Room No.', key: 'roomNumber', width: 20 },
        { header: 'Guest Name', key: 'guestName', width: 25 },
        { header: 'Comp/Agent', key: 'accountName', width: 25 },
        ...
        { header: 'No. Nights', key: 'numOfNights', width: 20 },
        { header: 'No. Stay', key: 'numOfStay', width: 20 },
        { header: 'Passport', key: 'passportNumber', width: 20 },
      ];

      worksheet.columns = columns;
      worksheet.addRow(['Report Name:', 'Past Guest Nationality Report']);
      ...
      workbook.xlsx.writeBuffer().then((buffer) => {
        ...
        saveAs(blob, `Past Guest Nationality Report_${formattedDate}.xlsx`);
      }).catch((error) => {
  ...
  return (
    <div>
      ...
      <Card>
        <CardHeader>
          <CardTitle tag='h4'>Past Guest Nationality</CardTitle>
        </CardHeader>
        <CardBody>
