...
const airportDrop = () => {
  ...
  const [columnDefs, setColumnDefs] = useState([
    ...
    {
      headerName: 'Departure Date ', field: 'departureDate', suppressSizeToFit: true, width: 140,
      cellRenderer: (params) => {
    ...
    {
      headerName: 'Drop Date', field: 'dropDate', suppressSizeToFit: true, width: 140,
      cellRenderer: (params) => {
        // Ensure the arrivalDate field exists in the row data        
        if (params.data && params.data.dropDate) {
          const formattedDate = Moment(params.data.dropDate).format("DD.MM.YYYY");
          return formattedDate;
        } else {
          return "";
        }
    ...
    { headerName: 'Drop Time', field: 'dropTime', suppressSizeToFit: true, width: 140 },
    { headerName: 'Station Code', field: 'dropStationCode', suppressSizeToFit: true, width: 140 },
  ...
  const onSubmit = data => {
    ...
    if (flag1 == true) {
      let res = fetchx(DASHBOARD_URL + "/getAirportDrops", {
        method: "POST",
        headers: { 'Content-Type': 'application/json' },
  ...
  const onBtnExport = () => {
    const params = {
      fileName: 'AirportDrop Report.xlsx',
      sheetName: 'Sheet1',
    };
  ...
  const printGrid = () => {
    if (filterFromDate && filterToDate) {
      ...
      const uniqueDepartureDates = Array.from(new Set(rowData1.map((row) => row.departureDate)));
      if (gridApi) {
        ...
        const textToCenter = "Airport Drop Report";
          ...
          .forEach((departureDate, index) => {
            const rowsForDate = rowData1
              ...
              .map((row) => {
                ...
                const formattedDropDate = formatDate(new Date(row.dropDate));
        ...
        pdf.text(`Filter From Departure Date: ${fromDate}`, 10, dateY + 10, { width: 500, align: 'left' });
        ...
        pdf.save('Airport Drop Report.pdf');
      } else {
      }
    }
  };
  ...
  const generateExcel = () => {
    if (filterFromDate && filterToDate) {
      ...
      const worksheet = workbook.addWorksheet('Airport Drops Report'); // Updated report name
      ...
      const columns = [
        ...
        { header: 'Departure Date', key: 'departureDate', width: 20 }, // Updated field
        { header: 'Drop Date', key: 'dropDate', width: 20 }, // Updated field
        { header: 'Drop Time', key: 'dropTime', width: 15 }, // Updated field
        { header: 'Station Code', key: 'dropStationCode', width: 15 }, // Updated field
        { header: 'Car Type', key: 'transportType', width: 15 }, // Updated field
        { header: 'Flight Details', key: 'dropCarrierCode', width: 20 }, // Updated field
        { header: 'Remarks', key: 'dropRemarks', width: 25 }, // Updated field
      ];
  
      worksheet.columns = columns;
  
      worksheet.addRow(['Report Name:', 'Airport Drops Report']); // Updated report name
      ...
      sanitizedData.forEach((row) => {
        worksheet.addRow({
          ...
          dropStationCode: row.dropStationCode,
          transportType: row.transportType,
          dropCarrierCode: row.dropCarrierCode,
          dropRemarks: row.dropRemarks,
        });
      });
      ...
      workbook.xlsx.writeBuffer().then((buffer) => {
        ...
        saveAs(blob, `Airport Drops Report_${formattedDate}.xlsx`); // Updated report name
      }).catch((error) => {
  ...
  return (
    <div>
      <Card>
        <CardHeader>
          <CardTitle tag='h4'>Airport Drops</CardTitle>
        </CardHeader>
        <CardBody>
