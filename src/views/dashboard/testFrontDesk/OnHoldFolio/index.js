...
const AvailabilityMatrix = () => {
    ...
    const [columnDefs, setColumnDefs] = useState([
        ...
        { headerName: 'roomNo', field: 'roomNumber', maxWidth: 120 },
        { headerName: 'guestName', field: 'guestName', width: 250 },
        { headerName: 'folio Bal', field: 'Balance', maxWidth: 130 },

        { headerName: 'bookingID', field: 'bookingID', maxWidth: 130 },

        { headerName: 'folio No', field: 'folioNumber', maxWidth: 130 },

        {
            headerName: 'Billing', cellRendererFramework: (params) => <Button color='primary' style={{ width: 140 }} onClick={() => {
                // setSelectedRow(params.data)
                localStorage.setItem('reservationID',params.data.reservationID)
                navigate('/dashboard/OnHoldFolioBilling')


            }} >Billing</Button>, suppressSizeToFit: true, maxWidth: 180
        },
        { headerName: 'CompanyName', field: 'companyName', width: 250 },

        { headerName: 'Cancelled', field: 'isCancelled', maxWidth: 130 },
    ...
    useEffect(() => {
      if(localStorage.getItem('reservationID')!=null){
        ...
        fetchx(API_URL + '/getAllOnHoldFolios?hotelID=1')
            .then(result => result.json())
            .then(rowData => {
                // for(let i=0; i<10;i++){
                console.log(rowData['data'])
  ...
  // let url = API_URL+'/getinvoices/PMSInvoice_'+sessionStorage.getItem('reservationID')+'_'+data.billNo+'.pdf'
    ...
    return (
        <div>
            <Card>
                <CardHeader>
                    <CardTitle tag="h4"><b>On Hold Folios</b></CardTitle>
                </CardHeader>
            </Card>
