...
function AllReservation(props) {
  ...
  const [columnDefs, setColumnDefs] = useState([
    ...
    {
      headerName: 'Billing', cellRendererFramework: (params) => <Button color='primary' style={{ width: 128 }} onClick={() => {
        localStorage.setItem('FolioTabNo', 0)
        setTimeout(() => { navigate('/dashboard/frontdesk/Billing') }, 1000)
      }} >Billing</Button>, suppressSizeToFit: true, maxWidth: 148, cellStyle: { textAlign: 'center' },
      cellClass: 'vertical-center',
    },
  ]);
...
`;
  ...
  const handleInputChange = (e) => {
    ...
    if (gridRef.current) {
      ...
      console.log('gridRef.current is not available');
    }
  ...
  useEffect(() => {
    fetchx(API_URL + "/getDefinteReservationForFrontDesk?hotelID=1")
      .then((result) => result.json())
      .then((rowData) => {
      ...
      <div>
        <Row>
          ...
          <Col md="2" sm="12" className="mb-1">
            <Label className="form-label" for="fullNameSearch">
              Group Name Filter:
            </Label>
            <Input
              type="text"
              id="roomNumberFilter10"
