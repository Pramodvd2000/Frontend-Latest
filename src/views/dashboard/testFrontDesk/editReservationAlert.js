...
const EditReservationAlert = (data1) => {
  ...
  useEffect(() => {
      fetchx(API_URL + `/getReservationAlerts?reservationID=${data1.data1.id}`)
        .then((result) => result.json())
        .then((resp) => {
          setResult(resp['data'][0]['reservationAlert']);
        });
    // }
  }, [data1.data1.id]); 
  ...
  const onSubmit = data => {
    setData(data)
    console.log("submitted", data1.data1.sharingID);
    if ( data.checkInAlert !== null) {
      let createmarketGroup = JSON.stringify({
        "reservationID": data1.data1.id,
        "sharingID":data1.data1.sharingID,
        "alertCode" : 1, 
        "reservationAlert": data.description,
             })

      let res = fetchx(API_URL+ `/updateReservationAlert` , {
        method: "PUT",
        ...
        if(res['status']==200){
          const swalInstance = MySwal.fire({
            text: 'Reservation Alert Edited Successfully!',
            icon: 'success',
  ...
  return (
    ...
    <div>     
        <Card>
      <CardHeader>
        <CardTitle tag='h4'>Edit Reservation Alerts</CardTitle>
      </CardHeader>
      <CardBody>
...
export default EditReservationAlert;