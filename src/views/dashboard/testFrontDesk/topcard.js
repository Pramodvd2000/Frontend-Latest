...
const HighlightedCount = ({ highlight, cardColor, count }) => (
    <span style={{ 
      backgroundColor: highlight ? 'yellow' : cardColor, 
      color: highlight ? 'black' : 'white', 
      padding: '3px 6px', 
      borderRadius: '4px', 
      fontWeight: 'bold', 
      marginLeft: 'auto' /* Align the count to the right side of the container */ ,
    }}>
      {count}
    </span>
);
const TopCards = () => {
  ...
  useEffect(() => {
    fetchx(API_URL + '/getAvailabilitryCount')
          ...
          <Col key={index}>
            <Card className="card" style={{ backgroundColor: card.color }}>
              <CardBody>
                ...
                <CardTitle tag="h4" className="cardTitle" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  {card.name} 
                  {' '}
