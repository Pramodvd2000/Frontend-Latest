...
let reservationID ={}
let roomNoarr1
const ValidationThirdPartyComponents = (folio) => {
  ...
  const getRowStyle = params => {
    console.log(params.data.hasOwnProperty('id'))
...
// pos_bill_number
  ...
  const getPOSInvoice = (data)=>{
    console.log(String(data.pos_bill_number).replaceAll("/","_").replaceAll("-",""))
      //Live server
  ...
  .then(resp => {
    ...
    setPOSInvURL(API_URL+'/imagepaths/POS_Invoice/POSInvoice_'+data.hotelID+'_'+(String(data.pos_bill_number).replaceAll("/","_").replaceAll("-",""))+'.pdf')
        setShowPOSInv(true)
  }
}
...
console.log( IDArr.length)
  ...
  const onDiscard = () => {
    ...
    const onCellValueChanged = useCallback( event => {
      ...
      // sharath
      if (event.colDef.field === 'extras' && folio['folio']!=0) {
        ...
        if(updatedValue == 'Cancel Transaction'){
          ...
          console.log(updatedValue)
              ...
              handleTransferTransaction('Others')
          ...
          if(event.data.transaction_type=='Bill'){
            console.log("Transction Type Bill")
            setTrnxSelected(event.data)
            // setTrxnIDArr(event.data.id)
  ...
  const cellClickedListener = useCallback( event => {
    ...
    // sharath
    if(event.column.colId === 'extras'){
      
      // const updatedColumnDefs = [...columnDefs];
      // const columnToUpdate = updatedColumnDefs.find(
        
      //   (column) => column.field === event.column.colId
      // );
      // if (columnToUpdate) {
      //  if(event.data.transaction_type == "Payments"){
      //     columnToUpdate.cellEditorParams = {
      //       values: ['Select','Cancel Transaction', 'Split Transaction'],
      //     };
      //   }else{
      //     columnToUpdate.cellEditorParams = {
      //       values: ['Select','Post Allowance', 'Transfer Transactions', 'Split Transaction', 'Cancel Transaction'],
      //     };
      //   }
      // }
  ...
  const UpdateSplitAmt =()=>{
    ...
    if(selectedOption=='Amount'){
      ...
      // }else{
      //   // alert("Split Amount selected is more then the transaction amount")
      //   console.log("error")
      //   seterrorMsg("Split Amount selected is more then the transaction amount")
      //   setshowErrorMsg(!showErrorMsg)
      //   document.getElementById('SplitAmount').value = "";
      //   document.getElementById('SplitWithTax').value = "";
      ...
      // if(TrnxSelected.base_amount >= amount){
        ...
        document.getElementById('SplitWithTax').value = parseFloat(amount)+parseFloat(taxAmt)
      // }else{
        // alert("Split percenetage is more then 100%")
        // console.log("error")
        // seterrorMsg("Split Amount selected is more then the transaction amount")
        // setshowErrorMsg(!showErrorMsg)
        // document.getElementById('Split').value = "";
  ...
  const AddAllowance =(data) =>{
    ...
    setReasonRemarks(data.reasonRemarks)
    console.log("================================>")
    console.log(document.getElementById('AllowanceWithTax').value)
...
let POSTdata
  ...
  const ConfirmTransferTransaction=(data)=>{
    ...
    .then(resp => {
        console.log(resp)
        if(resp.statusCode==200){
          console.log('Daily Posting present')
    ...
    .then(resp => {
      ...
      //       console.log('Heyyy')
           ...
           for(let i=0 ;i<tempRoomNoOptions.length ;i++){
            ...
            if(event === 'SameGuest' && TrnxSelected.roomNumber== tempRoomNoOptions[i]['value']){
              console.log('SameGuest')
...
const handleroomNoChange = (event) =>{
  ...
  console.log(roomNoarr1[event.value])
  ...
  console.log(guestNameOptions.length)
...
const handleFolioChange = (event) =>{
  console.log(event)
  console.log(folio)
  if(selectedOption ==='SameGuest' && folio['folio']==event.value){
  ...
  console.log(event.ReservationID)
...
fetchx(API_URL + "/getFolioCount", {
  ...
  const TransferTransaction =()=>{
    console.log(TrxnIDArr)
    console.log(FolioNumber)
    console.log(ReservationID)
    console.log(TrnxSelected.roomNumber,roomNo)
    // /TransferTransaction
  ...
  const SplitTransaction =() =>{
    ...
    console.log("================================>")
    console.log("Split Amount with tax " +document.getElementById('SplitWithTax').value)
    console.log("Split amount without Tax"+document.getElementById('SplitAmount').value)
    console.log("Amount or percentage Split " +document.getElementById('Split').value)
    console.log('TrnxID'+TrnxSelected.id)
    console.log('Trnx Base Amt'+TrnxSelected.base_amount)
    console.log('Trnx tax percentage'+TrnxSelected.tax_percentage)
...
<Modal isOpen={PostAllowance} toggle={() => setPostAllowance(!PostAllowance)} className='modal-dialog-centered modal-lg' onClosed={()=>{PostAllowanceReset()}}>
      ...
      <Modal isOpen={TransferTrnx} className='modal-dialog-centered modal-lg' onClosed={()=>{TransferTransactionReset()}}>
         ...
         {/* <Col md='4' sm='12' className='mb-1'>
          ...
          { console.log(roomNoOptions[0])}
          { console.log(typeof(roomNoOptions[0]))}
