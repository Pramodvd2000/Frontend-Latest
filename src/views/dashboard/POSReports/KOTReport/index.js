...
const KOTReport = () => {
  ...
  const columnDefs = [
    ...
    // { headerName: 'Kot Time', field: 'KOTTime', suppressSizeToFit: true, maxWidth: 300 },
    {
      headerName: 'KOT Date  ', field: 'KOTTime', suppressSizeToFit: true, maxWidth: 170, cellRenderer: (params) => {
        if (params.data && params.data.KOTTime) {
          const formattedDate = Moment(params.data.KOTTime).format("DD-MM-YYYY hh:mm:ss");
    ...
    {
      headerName: 'Quantity', field: 'Qty', suppressSizeToFit: true, maxWidth: 100, headerComponentFramework: CustomHeaderComponentQty,
      aggFunc: 'sum',
      // valueFormatter: formatNumber
    },
    {
      headerName: 'Total Amount', field: 'TotalAmount', suppressSizeToFit: true, maxWidth: 200, headerComponentFramework: CustomHeaderComponentTotaamount, aggFunc: 'sum',
      valueFormatter: formatNumber
    },
  ...
  const getKotData = data => {
    ...
    let res = fetchx(DASHBOARD_URL + "/getKotReport", {
      method: "POST",
      headers: { 'Content-Type': 'application/json' },
  ...
  const generateExcel = () => {
    if (filterFromDate && filterToDate) {
      ...
      const worksheet = workbook.addWorksheet('KOT Report');
  
      const columns = [
        { header: 'KOT No', key: 'KOTNo', width: 15 },
        { header: 'StoreName', key: 'RestaurantName', width: 20 },
        { header: 'KOT Date', key: 'KOTTime', width: 20 },
      ...
      worksheet.addRow(['Report Name:', 'KOT REPORT']);
      ...
      worksheet.columns.forEach((column, index) => {
        if (index !== 8 && index !== 7 && index !== 6 && index !== 5 && index !== 0) {
      ...
      workbook.xlsx.writeBuffer().then((buffer) => {
        ...
        saveAs(blob, `KOT_Report_${formattedDate}.xlsx`);
      }).catch((error) => {
  ...
  return (
    <div>
      <Card>
        <CardHeader>
          <CardTitle tag='h4'>KOT Report</CardTitle>
        </CardHeader>
        <CardBody>
