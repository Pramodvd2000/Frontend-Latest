...
import {Card,CardHeader,CardTitle,CardBody,Button,Modal,ModalBody,
...
const AvailabilityMatrix = () => {
  ...
  const loadPageData = (page) => {
    ...
    fetchx(API_URL + `/getAllPaidOutTransaction?hotelID=1&startRecord=${page}&endRecord=${11}`)
  ...
  const getReceipt =(transactionID)=>{
    ...
    // let url = 'http://122.166.2.21//PMS_Invoice/PaidOutReceipt/PaidOutReceipt_'+transactionID+'.pdf'

  //Live server
  // let url = API_URL+'/getinvoices/PaidOutReceipt_'+transactionID+'.pdf'
  ...
  if(!is_test){
    ...
    .then(resp => {
      ...
      setReceiptURL(API_URL+'/imagepaths/PMS_Invoice/PaidOutReceipt/PaidOutReceipt_'+transactionID+'.pdf')
      setShowReceiptPDF(true)
    }
}
  useEffect(() => {
  ...
  const SendEmail = (data) =>{
    ...
    fetchx(API_URL + "/sendemailtest", {
      ...
      body: JSON.stringify({
        ...
        date:Moment(CompleteData['date']).format("DD-MM-YYYY"),
  ...
  return (
    <div>
         <Card>
            <CardHeader>
              <CardTitle tag="h4"><b>All PaidOuts</b></CardTitle>
            </CardHeader>
          </Card>
          <div>
      ...
      <Modal isOpen={ShowReceiptPDF} toggle={() => setShowReceiptPDF(!ShowReceiptPDF)} style={{height:'200px'}} className='modal-dialog-centered modal-lg'>
        <ModalHeader className='bg-transparent' toggle={() => setShowReceiptPDF(!ShowReceiptPDF)}>Payment Receipt</ModalHeader>
