...
const Extras = ({ data1, eta, etd, toggleModal, otherDetails }) => {
  console.log("kjhgaskh")
  console.log(data1, eta, etd, otherDetails)
  ...
  useEffect(() => {
    fetchx(API_URL + '/getExtraDescription?hotelID=1')
      ...
      .then(resp => {
        ...
        // 🔥 get session extras
        const sessionExtras = JSON.parse(sessionStorage.getItem("extras")) || [];

        const sessionValues = sessionExtras.map(e => e.value);

        // ✅ filter out already selected extras
        const filteredData = apiData.filter(item => !sessionValues.includes(item.value));

        setExtraName(filteredData);
      });

  }, []);


  function refreshDropDownExtra() {
    fetchx(API_URL + '/getExtraDescription?hotelID=1')
      .then(result => result.json())
      .then(resp => {
        ...
        const filtered = resp.data.filter(item => !sessionValues.includes(item.value));

        setExtraName(filtered);
      });
  }
  ...
  const [columnDefs, setColumnDefs] = useState([
    // { headerName: 'Extra Code', field: 'extraCode', suppressSizeToFit: true, maxWidth: 140 },
    {
      ...
      valueGetter: (params) => {
        ...
        const match = extras.find(e => e.value === params.data.extraID || e.value === params.data.extraCode);
        return match ? match.label : params.data.extraCode;
      },
      width: 440
    },
  ...
  // useEffect(() => {
  //   fetchx(API_URL + `/getExtraForReservationBYID?reservationID=${sessionStorage.getItem("groupReservationID") || null}&operation=groupCreation`)
  //     .then(result => result.json())
  //     .then(rowData => {
  ...
  useEffect(() => {
    ...
    const formattedData = sessionExtras.map(e => ({
      extraID: e.value,
      extraCode: e.value
    }));
    ...
    toggleModal(formattedData); // if you still need this
  }, []);
  ...
  const onSubmit = (data) => {
    ...
    const selectedExtra = {
      value: data.extras.value,
      label: data.extras.label
    };

    let existingExtras = JSON.parse(sessionStorage.getItem("extras")) || [];

    // ✅ check uniqueness
    const alreadyExists = existingExtras.some(e => e.value === selectedExtra.value);

    if (!alreadyExists) {
      existingExtras.push(selectedExtra);
    }

    // 🔥 update sessionStorage
    sessionStorage.setItem("extras", JSON.stringify(existingExtras));
    handleReset()
    refreshDropDownExtra()
    // ✅ update grid instantly
    setRowData(prev => {
      const newRow = {
        extraID: selectedExtra.value,
        extraCode: selectedExtra.value // used for mapping label
      };
  ...
  const handleRemoveExtra = (data) => {
    ...
    setData(data);
    let extras = JSON.parse(sessionStorage.getItem("extras")) || [];

    // remove the deleted one
    extras = extras.filter(e => e.value !== filldata.extraID);

    sessionStorage.setItem("extras", JSON.stringify(extras));
    setRowData(prev =>
      prev.filter(row => Number(row.extraID) !== Number(filldata.extraID))
    );
    refreshDropDownExtra()
    setCofirmRemove(false)
    handleSuccess({ title: "Extra Removed Successfully", text: "The extra has been removed successfully." })
  };
