...
const Extras = ({ data1, eta, etd, toggleModal, otherDetails }) => {
  ...
  useEffect(() => {
    fetchx(API_URL + `/getExtraForReservation?reservationID=${sessionStorage.getItem("groupReservationID") || null}&operation=groupCreation`)
 ...
 const handleUpdateRates = async () => {
  ...
  const onSubmit = (data) => {
    ...
    let createExtra = JSON.stringify({
      ...
      reservationID: data1.companyid,
      operation: 'groupCreation',
      groupData: otherDetails
    });
    ...
    let res = fetchx(API_URL + "/addReservationExtrasByExtraID", {
      ...
      .then((res) => {
        ...
        if (res.statusCode === 200) {
          sessionStorage.setItem('groupReservationID', res.data)
          handleReset()
          fetchx(API_URL + `/getExtraForReservationBYID?reservationID=${sessionStorage.getItem("groupReservationID") || null}&operation=groupCreation`)
            .then(result => result.json())
            .then(rowData => {
