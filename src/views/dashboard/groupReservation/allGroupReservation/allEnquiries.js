...
const AllEnquiryReservations = () => {
    ...
    useEffect(() => {
        fetchx(API_URL + "/getAllGroupReservations?filterKey=Enquiry")
            .then((result) => result.json())
            .then((rowData) => {
    ...
    // On click tentative function call
    const TentativFunctionCall = () => {
        setOptions(!options)
        getLatestData(sessionStorage.getItem('groupReservationID'))
    }
            ...
            <div>
                {reservationData && <Modal isOpen={options} toggle={() => setOptions(!options)} className="modal-xl">
                    ...
                    <ModalBody className="pb-3 px-sm-2 mx-20">
                        ...
                        {filldata && 
                            <InventoryEntryTable filldata={filldata} closeInventoryModal={closeInventoryModal} tabState={'enquiry'}/>
                        } 
                        <br></br>
                        {/* <div align='end'>
...
<div>
        ...
        <ModalHeader className="modal-lg" toggle={() => { setOptions1(!options1); }}>
            ...
            <div>
                {reservationData && <Modal isOpen={roomCountChange} toggle={() => setRoomCountChange(!roomCountChange)} className="modal-xl">
                    ...
                    <ModalBody className="pb-3 px-sm-2 mx-20">
                        <br></br>
                        <p><b>Note:- Click on cells to enter the data and don't forget to save the data before going to next row.</b></p>
                        <p>
                            &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                            <Row>
                                <Col md='3' sm='12'>
  ...
  {reservationData && reservationData.status === 'Enquiry' && <div onClick={extrasModify} className="hoverUnderline" >
                                            Modify Extras
                                        </div>}
                                    </Col>
...
export default AllEnquiryReservations