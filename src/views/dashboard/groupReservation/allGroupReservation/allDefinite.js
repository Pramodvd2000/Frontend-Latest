...
import ModifyBookingInfoForRes from './bookingInfoModifyForRes';
...
import CancelIndividualReservation from './cancelReservation';

const AllDefiniteReservations = () => {
    ...
    const [bookingInfoModRes, setBookingInfoModRes] = useState();
 ...
 useEffect(() => {
    ...
    const [columnDefs3] = useState([
        ...
        },
        {
            ...
            // autoHeaderHeight: true,
            // wrapHeaderText: true,
            valueGetter: (params) => (params.data.isMain === 0 ? null : params.data.numberOfAdults),
        },
        {
        ...
        {
            headerName: "Billing Instruction",
            field: "billingInstruction",
            maxWidth: 150,
            autoHeaderHeight: true,
            wrapHeaderText: true,
        ...
        //             disabled={!!params.data.bookingID} 
        //         >
        //             Confirm Booking
        //         </Button>
        //     ),
    ...
    const gridOptions = {
        getRowStyle: (params) => {
            if (params.data.isMain === 0) {
                return { backgroundColor: "#F1C40F" }; // Inline style for row highlight
            }
    ...
    useEffect(() => {
        fetchx(API_URL + "/getAllGroupReservations?filterKey=Definite")
            ...
            if (clickedElement.tagName === 'BUTTON') {
                ...
                if ((buttonName === 'Assign Room' || buttonName === 'View Reg Card' || buttonName === 'Check In' || buttonName === 'Add Sharer') && event.data.bookingID == undefined) {
                    ...
                    if (event.data.roomNumber == null) {
                        return handleError("First assign the room")
                    }
                    if (event.data.reservationStatus == 'Checked In' || event.data.reservationStatus == 'Due Out') {
                        return handleError("Guest Already Checked In")
                    }
                    fetchx(API_URL + `/getReservationForFrontDeskGroup?bookingID=${event.data.bookingID}`)
                        .then(result => result.json())
                        .then(resp => {
                    ...
                    fetchx(API_URL + `/getReservationForFrontDeskByResID?reservationID=${event.data.id}`)
                        ...
                        .then(resp => {
                            if (resp.data && resp.data[0]) {
                                ...
                                handleError("No reservation data found");
                            }
                        })
                        .catch(error => {
                            handleError("Error fetching reservation data");
    ...
    //     fetchx(API_URL + `/insertGroupExcelData`, {
    //         method: "POST",
    //         headers: { 'Content-Type': 'application/json' },
    //         body: groupData
    //     })
    ...
    //             navigate(''); // Navigate when "OK" is clicked
    //         }
    //     });
    ...
    //                 // MySwal.fire({
    //                 //     title: `Reservation Creation successful for ${reservationData.groupName}!`,
    //                 //     text: `API call completed with ${countdown} seconds left.`,
    //                 //     icon: 'success',
    //                 // });
    //             }
    ...
    //                     setTempData(gridData.data); // Update grid row data
    //                     setFileName("");
    //                     setFileData([]);
    //                     // navigate("")
    //                 });
    ...
    const handleSuccessOfCancel = async (message) => {
        await MySwal.fire({
            ...
            text: `Successfully cancelled group booking`,
            icon: 'success',
        });
    ...
    const handleSubmit2 = (row) => {
        ...
        if ((row.numberOfPickedRooms === undefined) || (row.baseAmount === undefined) || (row.extraAdultPrice === undefined) || (row.extraChildPrice === undefined) || (row.packageRate === undefined)) {
            handleError('Without data, it is not allowed');
        }
    ...
    const openIndvGroupRes = (reservationData) => {
        if (reservationData.status === 'Cancelled') {
            return handleError("This operation is not allowed")
        }

        setOpenCancelIndividualBooking(!openCancelIndividualBooking)
    }
    ...
    //         { header: 'Arrival_date', key: 'Arrival_date', width: 15 },
    //         { header: 'Departure_date', key: 'Departure_date', width: 15 },
    //         {header: 'Number_Of_Adults', key: 'Number_Of_Adults', width: 15}, 
    //         { header: 'Salutation', key: 'Salutation', width: 10 },
    ...
    //         { header: 'Salutation_Sharer1', key: 'Salutation_Sharer1', width: 10 },
    //         { header: 'First_Name_Sharer1', key: 'First_Name_Sharer1', width: 15 },
    //         { header: 'Last_Name_Sharer1', key: 'Last_Name_Sharer1', width: 15 },
    //         { header: 'Email_ID_Sharer1', key: 'Email_ID_Sharer1', width: 25 },
    //         { header: 'Phone_Number_Sharer1', key: 'Phone_Number_Sharer1', width: 15 },
    //         { header: 'Salutation_Sharer2', key: 'Salutation_Sharer2', width: 10 },
    //         { header: 'First_Name_Sharer2', key: 'First_Name_Sharer2', width: 15 },
    //         { header: 'Last_Name_Sharer2', key: 'Last_Name_Sharer2', width: 15 },
 ...
 function AssignRoomOnRoomingListUpload() {
  ...
  // const [options3, setOptions3] = useState();
    ...
    const updateInstructions = (data) => {
        fetchx(API_URL + `/updateCommentsAndBilling`, {
            method: "POST",
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
...
>
  ...
  >
    ...
    />
                            ...
                            <Card style={{ backgroundColor: "#F2E5D9" }}>
                                <Row className="cardBody">
                                    ...
                                    <Col md="6" sm="12" className="mb-1">
                                        ...
                                        {reservationData && reservationData.status === 'Definite' && <div onClick={() => {
            ...
            // style={{ maxWidth: '1400px', maxHeight: '60vh' }}
            >
                <ModalHeader toggle={handleClose} className="modal-lg">
                    Modify Booking Information
                </ModalHeader>
                <ModalBody className="modal-lg">
            ...
            >
                ...
                <ModalBody className="modal-lg">
                    <GroupModificationLogs reservationData={reservationData} />
    ...
    />
                    ...
                    <Card>
                        <CardBody>
                            <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: '20px' }}>
                                <Button color="primary" onClick={() => openIndvGroupRes(reservationData)}>
            ...
            {reservationData !== "" &&
                <Modal
                    ...
                    toggle={() => setOpenCancelIndividualBooking(!openCancelIndividualBooking)}
                    // toggle={openCancelIndividualBooking}
                    className="modal-dialog-centered modal-lg"
                // style={{ maxWidth: '700px', width: '700px', margin: 'auto' }}
                >
                    <ModalHeader toggle={() => setOpenCancelIndividualBooking(!openCancelIndividualBooking)}>
                        Select Reservations
                    </ModalHeader>
                    <ModalBody>
                        <CancelIndividualReservation reservationData={reservationData} />
                    </ModalBody>
                </Modal>
            }
        </div>
...
export default AllDefiniteReservations
