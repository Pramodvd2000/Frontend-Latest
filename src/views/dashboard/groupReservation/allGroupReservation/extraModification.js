...
const Extras = ({ data1, refreshAPI }) => {
  ...
  useEffect(() => {
    fetchx(API_URL + `/getExtraForReservation?reservationID=${data1.id}&operation='group'`)
  ...
  useEffect(() => {
    fetchx(API_URL + `/getExtraForReservationBYID?reservationID=${data1.id}?operation='group'`)
      .then(result => result.json())
      .then(rowData => {
    ...
    let res = fetchx(API_URL + "/addReservationExtrasByExtraID", {
      ...
      .then((res) => {
        ...
        if (res.statusCode === 200) {
          handleReset()
          fetchx(API_URL + `/getExtraForReservationBYID?reservationID=${data1.id}`)
            .then(result => result.json())
            .then(rowData => {
              setRowData(rowData['data'])
